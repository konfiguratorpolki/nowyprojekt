// --- Zmienne globalne i elementy DOM ---
        ...
        let _idleResetCam = null;      // domyślna pozycja kamery (zapisana przy otwarciu)
        let _idleTimer = null;         // setTimeout handle
        const initialCameraPosition = { x: 8.2551, y: 5.0487, z: 13.1641 };
        ...
        const SHELF3D_CAM_PER_SIZE = {
            '44_40': { x: 7.0777, y: 4.3286, z: 11.2866, tx: 0, ty: 0, tz: 0 },
            '44_60': { x: 8.2551, y: 5.0487, z: 13.1641, tx: 0, ty: 0, tz: 0 },
            '44_80': { x: 10.6685, y: 6.5248, z: 17.0126, tx: 0, ty: 0, tz: 0 },
        };
        ...
        function shelf3dFitCameraPredict(widthCm, heightCm, dur, optDir) {
            ...
            camera.position.set(_np.x, _np.y, _np.z); controls.update();
        }
        ...
        function shelf3dAutoFitCamera(dur, optDir, onlyZoomOut) {
            ...
            // onlyZoomOut: pozwól tylko oddalić kamerę (gdy półka się nie mieści),
            // nigdy nie przybliżaj — inaczej po zbudowaniu model jest niepotrzebnie powiększany.
            if (onlyZoomOut) {
                const _curDist = camera.position.distanceTo(controls.target);
                if (_dist <= _curDist + 0.01) return;
            }
            const _dir = optDir
                ? optDir.clone().normalize()
                : new THREE.Vector3().subVectors(camera.position, controls.target).normalize();
        ...
        // Natychmiastowe (bez animacji) dopasowanie kamery do całej półki —
        // używane przy zmianie rozmiaru okna, żeby model nigdy nie był ucinany,
        // a jednocześnie nie był sztucznie powiększany. Zachowuje kąt patrzenia.
        function shelf3dRefitCameraInstant() {
            if (!camera || !controls || !shelfGroup) return;
            const _box = new THREE.Box3().setFromObject(shelfGroup);
            ...
            const _fovV = camera.fov * Math.PI / 180;
            const _fovH = 2 * Math.atan(Math.tan(_fovV / 2) * camera.aspect);
            const _pad = window.innerWidth < 768 ? 1.38 : 1.32;
            const _dV = (_size.y / 2) / Math.tan(_fovV / 2) * _pad;
            const _dH = (_size.x / 2) / Math.tan(_fovH / 2) * _pad;
            const _dist = Math.max(_dV, _dH);
            const _dir = new THREE.Vector3().subVectors(camera.position, controls.target).normalize();
            const _np = _center.clone().add(_dir.multiplyScalar(_dist));
            camera.position.copy(_np);
            controls.update();
        }
        // Na mobile kolumna ikon (60px, prawa krawędź) nakłada się na canvas i zasłaniała
        // prawą część półki. Rezerwujemy ten pas w samym renderze (viewport), więc model
        // mieści się OBOK ikon, a nie pod nimi. Aspect dopasowany do węższego obszaru,
        // żeby nic nie było ucinane ani zniekształcone.
        const SHELF3D_MOBILE_ICON_RESERVE = 0; // 3D na pełną szerokość; ikony są półprzezroczyste i bledną przy dotyku modelu
        function shelf3dApplyMobileViewport() {
            if (!renderer || !camera || !renderer.domElement) return false;
            const canvas = renderer.domElement;
            const W = canvas.clientWidth || canvas.width;
            const H = canvas.clientHeight || canvas.height;
            if (W <= 0 || H <= 0) return false;
            const isMobile = window.innerWidth < 768;
            const panelActive = typeof shelfContainer !== 'undefined' && shelfContainer && shelfContainer.classList.contains('active');
            let asp;
            if (isMobile && panelActive) {
                const vw = Math.max(W - SHELF3D_MOBILE_ICON_RESERVE, 40);
            ...
            if (isFinite(asp) && asp > 0 && Math.abs(camera.aspect - asp) > 0.0005) {
                camera.aspect = asp;
                camera.updateProjectionMatrix();
            }
            return isMobile && panelActive;
        }
        let currentAnimationTimeline = null;
        let lastValidConfig = {};
        // Blokada kamery na mobile podczas animacji budowania półki —
        // wymusza pozycję każdą klatkę zanim OrbitControls zdąży nadpisać.
        let _mobileBuildLockCam = null;
    ...
    function applyShelfConfigurationFromGallery(galleryIndex, imageIndex) {
            ...
            // Odśwież camera.aspect do aktualnych wymiarów canvasu (może być nieaktualne po snapshocie lub przełączeniu widoku)
            if (typeof onWindowResize === 'function') onWindowResize();
            updatePreview(true);
            updateOrderSummary();
        }
...
function updateDividerIconsVisibility() { if (!shelfContainer || !shelfTypeSelect || !mugShelfDividerIconsContainer || !mobileMugShelfMountPanel || !dividerTooltip) return; const isMobile = window.innerWidth < 768; const isMugShelf = shelfTypeSelect.value === 'mug_shelf'; const is3dPanelActive = shelfContainer.classList.contains('active'); if (isMobile && isMugShelf && is3dPanelActive) { mugShelfDividerIconsContainer.classList.add('visible'); mobileMugShelfMountPanel.classList.add('visible'); { var _mpi = document.getElementById('mobilePanelIcons'); if (_mpi) _mpi.classList.add('mug-bar-reserve'); } if (!dividerTooltip.dataset.shown) { dividerTooltip.classList.add('visible'); dividerTooltip.dataset.shown = 'true'; setTimeout(() => { dividerTooltip.classList.remove('visible'); }, 5000); } if (typeof updateMobileMountStripStates === "function") updateMobileMountStripStates(); } else { mugShelfDividerIconsContainer.classList.remove('visible'); mobileMugShelfMountPanel.classList.remove('visible'); { var _mpi2 = document.getElementById('mobilePanelIcons'); if (_mpi2) _mpi2.classList.remove('mug-bar-reserve'); } }
    ...
    if (_dimLabel) {
        ...
        function filterWidthsByHeight() { const h = heightSelect.value; const wideWidths = ['70', '80', '84']; const hideWide = (h !== '' && h !== '40' && h !== '60'); Array.from(widthSelect.options).forEach(opt => { if (wideWidths.includes(opt.value)) { opt.style.display = hideWide ? 'none' : ''; opt.disabled = hideWide; } }); if (hideWide && wideWidths.includes(widthSelect.value)) { widthSelect.value = '60'; checkCustomWidth(); } const _slider = document.getElementById('customWidthInput'); if (_slider) { const _newMax = (h === '40' || h === '60') ? 83 : 59; _slider.max = _newMax; if (parseInt(_slider.value) > _newMax) { _slider.value = _newMax; const _disp = document.getElementById('customWidthDisplay'); if (_disp) _disp.textContent = _newMax + ' cm'; } } }
        function handleDimensionChange() { activeGalleryConfig = null; const _anyEmpty = !heightSelect.value || !widthSelect.value || (depthSelect && !depthSelect.value); if (_anyEmpty && typeof _dimSegments !== 'undefined') { _dimSegments = []; const _ds = document.getElementById('dimOverlaySvg'); const _dg = _ds && _ds.querySelector('#dimOverlayG'); if (_dg) _dg.innerHTML = ''; } if (shelfTypeSelect.value !== 'mug_shelf') { filterWidthsByHeight(); updateDepthOptions(); updateShelfCountOptions(); } else { if(dividersTopCheckbox) dividersTopCheckbox.checked = false; if(dividersMiddleCheckbox) dividersMiddleCheckbox.checked = false; if(dividersBottomCheckbox) dividersBottomCheckbox.checked = false; const _dh = document.getElementById('mugDividerHint'); if (_dh) _dh.style.display = ''; if (typeof updateDividerIconActiveStates === 'function') updateDividerIconActiveStates(); } if (typeof showOrHideCustomShelfToggle === 'function') showOrHideCustomShelfToggle(); if (typeof customShelfPositionEnabled !== 'undefined' && customShelfPositionEnabled) { disableCustomPositions(); } updatePreview(); updateOrderSummary(); }
      ...
      function handleShelfTypeChange(isLoading = false) {
             ...
             if (!isLoading) {
                // Auto-ustaw kolor dąb craft przy wyborze każdego rodzaju półki
                sideColorSelect.value = '#8B5A2B';
                shelfColorSelect.value = '#8B5A2B';
                updateSwatchDisplay('sideColor', 'sideColorSwatchDisplay');
                updateSwatchDisplay('shelfColor', 'shelfColorSwatchDisplay');
                if (typeof shelf3dApplyColorLight === 'function') shelf3dApplyColorLight('#8B5A2B');
             }
            ...
            if (shelfType !== 'mug_shelf') {
             ...
             if (shelfType === 'standing' || shelfType === 'hanging') {
                ...
                // 84 cm dostępna również dla półek wiszącej i stojącej
                if (!isLoading) {
                 ...
                 if (!isLoading) {
                     ...
                     if (_hint) { _hint.style.visibility = 'visible'; _hint.style.opacity = '1'; }
                 }
                ...
                if (customType) {
                    ...
                    // Wyczyść stare strzałki wymiarów z poprzedniej półki
                    if (typeof _dimSegments !== 'undefined') { _dimSegments = []; }
                    const _svgCl = document.getElementById('dimOverlaySvg');
                    const _gCl = _svgCl && _svgCl.querySelector('#dimOverlayG');
                    if (_gCl) _gCl.innerHTML = '';
                    ...
                    // Wysokość wzoru może być niestandardowa (np. 30 cm) — dorzuć opcję, by select mógł ją przyjąć (inaczej value="" → NaN → brak 3D)
                    const _hVal = String(customType.height);
                    if (heightSelect && !Array.from(heightSelect.options).some(o => o.value === _hVal)) {
                        const _hOpt = document.createElement('option');
                        _hOpt.value = _hVal; _hOpt.textContent = _hVal + ' cm';
                        heightSelect.appendChild(_hOpt);
                    }
                    heightSelect.value = _hVal;
                    ...
                    // Zawsze ustaw dąb craft jako domyślny kolor przy wyborze wzoru
                    if (!isLoading) {
                        sideColorSelect.value  = '#8B5A2B';
                        shelfColorSelect.value = '#8B5A2B';
                        updateSwatchDisplay('sideColor',  'sideColorSwatchDisplay');
...
function updateModularIconActiveStates() {
        ...
        function updateModularIconsVisibility() {
            ...
            const _wasModular = shelfContainer.classList.contains('modular-mode');
            if (isMobile && isModularShelf && is3dPanelActive) {
                modularShelfIconsContainer.classList.add('visible');
                shelfContainer.classList.add('modular-mode');
                // Przebuduj renderer do nowej szerokości canvasu (pełna szerokość ekranu)
                if (!_wasModular && typeof onWindowResize === 'function') {
                    requestAnimationFrame(() => onWindowResize());
                }
            } else {
                modularShelfIconsContainer.classList.remove('visible');
                if (_wasModular) {
                    shelfContainer.classList.remove('modular-mode');
                    // Przywróć renderer do normalnej szerokości
       ...
       function handleMugShelfWidthChange() { const currentMugShelfWidth = widthSelect.value; let heightChangedProgrammatically = false; if (shelfTypeSelect && shelfTypeSelect.value === 'mug_shelf' && widthSelect && heightSelect) { const sixtyCmHeightOption = Array.from(heightSelect.options).find(opt => opt.value === '60'); const fortyCmHeightOption = Array.from(heightSelect.options).find(opt => opt.value === '40'); if (currentMugShelfWidth === '84') { if (sixtyCmHeightOption) { sixtyCmHeightOption.disabled = true; sixtyCmHeightOption.style.textDecoration = 'line-through'; sixtyCmHeightOption.style.color = '#999'; } if (heightSelect.value === '60' && sixtyCmHeightOption && sixtyCmHeightOption.disabled) { if (fortyCmHeightOption && !fortyCmHeightOption.disabled) { heightSelect.value = '40'; heightChangedProgrammatically = true; } } } else { if (sixtyCmHeightOption) { sixtyCmHeightOption.disabled = false; sixtyCmHeightOption.style.textDecoration = ''; sixtyCmHeightOption.style.color = ''; } } } if(dividersTopCheckbox) dividersTopCheckbox.checked = false; if(dividersMiddleCheckbox) dividersMiddleCheckbox.checked = false; if(dividersBottomCheckbox) dividersBottomCheckbox.checked = false; if (heightChangedProgrammatically && typeof handleMugShelfHeightChange === "function") { /* handleMugShelfHeightChange wywołuje updatePreview — nie wywołuj ponownie */ handleMugShelfHeightChange(); updateOrderSummary(); } else { updatePreview(true); updateOrderSummary(); } }
        ...
        // Inteligentne ustawienie światła:
        // 1. Najpierw sprawdza tabelę kombinacji (boki_półki) — najdokładniejsze
        // 2. Potem kolory z priorytetem (np. dąb craft zawsze wygrywa)
        // 3. Na końcu kolor półek jako fallback
        function shelf3dApplyColorLightSmart() {
            ...
            if (SHELF3D_LIGHT_PER_TYPE[_typeVal]) {
                window._shelf3dApplyLightVals && window._shelf3dApplyLightVals(SHELF3D_LIGHT_PER_TYPE[_typeVal]);
                if (scene && window._rglBgDefault && window._rglBgWhite) { const _hw=_sideVal==='#FFFFFF'||_shelfVal==='#FFFFFF'; scene.background=_hw?window._rglBgWhite:window._rglBgDefault; }
                return;
            }
            // 1. Combo
            const _comboKey = _sideVal + '_' + _shelfVal;
            if (SHELF3D_LIGHT_PER_COMBO[_comboKey]) {
                window._shelf3dApplyLightVals && window._shelf3dApplyLightVals(SHELF3D_LIGHT_PER_COMBO[_comboKey]);
                if (scene && window._rglBgDefault && window._rglBgWhite) { const _hw=_sideVal==='#FFFFFF'||_shelfVal==='#FFFFFF'; scene.background=_hw?window._rglBgWhite:window._rglBgDefault; }
                return;
            }
            // 2. Priorytet koloru
            for (const _pc of SHELF3D_LIGHT_PRIORITY) {
                const _pcu = _pc.toUpperCase();
                if (_sideVal === _pcu || _shelfVal === _pcu) {
                    const _cfg = SHELF3D_LIGHT_PER_COLOR[_pcu];
                    if (_cfg && window._shelf3dApplyLightVals) { window._shelf3dApplyLightVals(_cfg); if(scene&&window._rglBgDefault&&window._rglBgWhite){const _hw=_sideVal==='#FFFFFF'||_shelfVal==='#FFFFFF';scene.background=_hw?window._rglBgWhite:window._rglBgDefault;} return; }
                }
            }
            // 3. Fallback — kolor półek
            const _cfg = SHELF3D_LIGHT_PER_COLOR[_shelfVal] || SHELF3D_LIGHT_PER_COLOR[_sideVal];
            if (_cfg && window._shelf3dApplyLightVals) window._shelf3dApplyLightVals(_cfg);
            // Przełącz tło 3D zależnie od kolorów
            if (scene && window._rglBgDefault && window._rglBgWhite) {
                const _hasWhite = _sideVal === '#FFFFFF' || _shelfVal === '#FFFFFF';
                scene.background = _hasWhite ? window._rglBgWhite : window._rglBgDefault;
                // Delikatne cienie: biała półka otrzymuje cienie ale hemi wypełnia je miękko
                if (shelfGroup) {
                    shelfGroup.traverse(o => {
                        if (o.isMesh && !o.name?.startsWith('__')) {
                            o.receiveShadow = true;
                        }
                    });
                }
                // Kontrast canvasa: biała półka potrzebuje wyższego kontrastu żeby krawędzie były widoczne
                if (renderer && renderer.domElement) {
                    const _c = _hasWhite ? 1.22 : SHELF3D_CANVAS_CONTRAST;
        ...
        let SHELF3D_MAT_NORMAL      = 1.10;
        let SHELF3D_MAT_ENV         = 1.15;
        ...
        function shelf3dInitTextures(scene, _renderer) {
            ...
            // Generuj subteLną teksturę szumu dla białego lakieru (brak pliku — canvas 2D)
            (function() {
                const _wc = document.createElement('canvas');
                _wc.width = 512; _wc.height = 512;
                const _wx = _wc.getContext('2d');
                _wx.fillStyle = '#fafaf8';
                _wx.fillRect(0, 0, 512, 512);
                const _id = _wx.getImageData(0, 0, 512, 512);
                const _d = _id.data;
                for (let i = 0; i < _d.length; i += 4) {
                    const n = (Math.random() - 0.5) * 14;
                    _d[i]   = Math.min(255, Math.max(220, _d[i]   + n));
                    _d[i+1] = Math.min(255, Math.max(220, _d[i+1] + n * 0.92));
                    _d[i+2] = Math.min(255, Math.max(215, _d[i+2] + n * 0.82));
                }
                _wx.putImageData(_id, 0, 0);
                const _wt = new THREE.CanvasTexture(_wc);
                _wt.wrapS = _wt.wrapT = THREE.RepeatWrapping;
                _wt.encoding = THREE.sRGBEncoding;
                _shelf3dTextures['__white__'] = _wt;
            })();
        ...
        function shelf3dMakeMaterialForBoard(type, w, h, d) {
            ...
            const matParams = {
                ...
                ...(nClone ? { normalMap: nClone, normalScale: new THREE.Vector2(SHELF3D_MAT_NORMAL, SHELF3D_MAT_NORMAL) } : {}),
                ...(rClone ? { roughnessMap: rClone } : {}),
                roughness:          _mc.rough,
                metalness:          _mc.metal,
        ...
        function _shelf3dApplyEdge() {
            ...
            shelfGroup.traverse(o => {
                ...
                if (o.name && (o.name.startsWith('dimensionLabel_') || o.name.startsWith('mugHeightArrow_') || o.name.startsWith('mugWidthArrow_'))) return;
                ...
                if (_edge <= 0.005) {
                    ...
                    if (Array.isArray(o.material)) {
                        ...
                        if (old4 && old4 !== baseMat) old4.dispose();
                        o.material = baseMat;
                        delete o.userData._edgeBaseMat;
                    }
                    return;
                }
                // Zapamiętaj bazowy materiał
                o.userData._edgeBaseMat = baseMat;
                ...
                // 6 materiałów: +X, -X, +Y, -Y, +Z(przód=oklein), -Z
                o.material = [baseMat, baseMat, baseMat, baseMat, edgeMat, baseMat];
            });
        }
        ...
        function shelf3dReapplyMaterials() {
            ...
            shelfGroup.traverse(child => {
                ...
                if (child.userData && child.userData.shelfMatInfo) {
                    ...
                    const n = child.name || '';
                    // leftSide/rightSide i przegródki pionowe z edytora → kolor boków
                    // divider_ (przegródki kubkowe) → kolor półek
        ...
        function init3D() { const container = threeJsCanvasWrapper; if (!container) { console.error("Three.js canvas wrapper (#threeJsCanvasWrapper) not found."); return; } scene = new THREE.Scene(); (function(){
                ...
                _bgCanvas.width = 512; _bgCanvas.height = 512;
                const _bgCtx = _bgCanvas.getContext('2d');
                const _bgGrad = _bgCtx.createLinearGradient(0, 0, 0, 512);
                _bgGrad.addColorStop(0.00, '#f5f4f2');
                _bgGrad.addColorStop(0.45, '#f0ede8');
                _bgGrad.addColorStop(0.72, '#e8e2d9');
                _bgGrad.addColorStop(1.00, '#ddd6cc');
                _bgCtx.fillStyle = _bgGrad;
                _bgCtx.fillRect(0, 0, 512, 512);
                const _vGrad = _bgCtx.createRadialGradient(256,256,120,256,256,380);
                _vGrad.addColorStop(0, 'rgba(0,0,0,0)');
                _vGrad.addColorStop(1, 'rgba(0,0,0,0.07)');
                _bgCtx.fillStyle = _vGrad;
                _bgCtx.fillRect(0, 0, 512, 512);
                window._rglBgDefault = new THREE.CanvasTexture(_bgCanvas);

                window._rglBgWhite = new THREE.Color(0xffffff);
        ...
        function shelf3dInitCameraHelper() {
            ...
            btn.style.cssText = 'position:fixed;bottom:214px;right:15px;z-index:99999;width:42px;height:42px;border-radius:50%;background:rgba(255,255,255,0.92);backdrop-filter:blur(10px);-webkit-backdrop-filter:blur(10px);color:#a78bfa;border:1.5px solid #a78bfa;cursor:pointer;display:flex;align-items:center;justify-content:center;box-shadow:0 2px 8px rgba(0,0,0,0.10);transition:transform 0.15s;font-size:20px;line-height:1;';
            btn.innerHTML = '⚙️';
        ...
        function shelf3dInitDebugPanel() {
            ...
            btn.style.cssText = 'position:fixed;bottom:110px;right:15px;z-index:99999;width:42px;height:42px;border-radius:50%;background:rgba(255,255,255,0.92);backdrop-filter:blur(10px);-webkit-backdrop-filter:blur(10px);color:#f59e0b;border:1.5px solid #f59e0b;cursor:pointer;display:flex;align-items:center;justify-content:center;box-shadow:0 2px 8px rgba(0,0,0,0.10);transition:transform 0.15s;';
            btn.onmouseenter = () => btn.style.transform='scale(1.12)';
            btn.onmouseleave = () => btn.style.transform='scale(1)';
            document.body.appendChild(btn);
            const panel = document.createElement('div');
            ...
            function applyShadow(){ SHELF3D_SHADOW_OPACITY=cfg.shadowOp; const _sf=scene.getObjectByName('__shadowFloor__'); if(_sf && _sf.material){ _sf.visible=true; _sf.material.opacity=cfg.shadowOp; _sf.material.needsUpdate=true; } }
...
'// Oświetlenie\nHemi: '+cfg.hemiInt.toFixed(2)+'; Key: '+cfg.dir1Int.toFixed(2)+'; Fill: '+cfg.dir2Int.toFixed(2)+'; Rim: '+cfg.rimInt.toFixed(2)+';';
        ...
        function shelf3dInitLightPanel() {
            let _az=137, _el=50, _dist=18, _int=4.00, _temp=83;
            let _hemi=0.76, _rim=0.92, _rimAz=217;
            let _shOp=0.11, _shBlur=3, _blOp=0;
            ...
            // Override SAMEGO kąta światła (azymut/elewacja) — bez ruszania temperatury/intensywności (te zostają od koloru)
            window._shelf3dApplyLightAngles = function(az, el){ if(typeof az==='number') _az=az; if(typeof el==='number') _el=el; _updMain(); try{ _syncSliders(); }catch(_e){} };
            // Tabela kąta światła zależna od rozmiaru półki (wysokość_głębokość w cm). Na razie: 80/10 i 80/15.
            window._shelf3dApplyLightForSize = function(height, depth){
                var _hEl=document.getElementById('height'), _dEl=document.getElementById('depth');
                var hv=(height!=null?height:(_hEl&&_hEl.value)), dv=(depth!=null?depth:(_dEl&&_dEl.value));
                var h=parseInt(hv,10), d=parseInt(dv,10);
                var T={ '80_10':{az:146,el:39}, '80_15':{az:159,el:30}, '60_10':{az:154,el:42}, '60_15':{az:150,el:27}, '40_10':{az:159,el:43}, '40_15':{az:159,el:30} };
                var e=T[h+'_'+d];
                if(e && window._shelf3dApplyLightAngles) window._shelf3dApplyLightAngles(e.az, e.el);
            };
            const btn=document.createElement('button');
            btn.id='shelf3dLightBtn';
            btn.title='Panel oświetlenia (Ctrl+Shift+L)';
            btn.style.cssText='position:fixed;bottom:162px;right:15px;z-index:99999;width:42px;height:42px;border-radius:50%;background:rgba(255,255,255,0.92);backdrop-filter:blur(10px);-webkit-backdrop-filter:blur(10px);color:#60a5fa;border:1.5px solid #60a5fa;cursor:pointer;display:flex;align-items:center;justify-content:center;box-shadow:0 2px 8px rgba(0,0,0,0.10);transition:transform 0.15s;font-size:19px;line-height:1;';
            ...
            panel.style.cssText='display:none;position:fixed;top:50%;left:50%;transform:translate(-50%,-50%);z-index:1000000;background:#0e1726;color:#f0f0f0;padding:22px;border-radius:16px;font-family:ui-monospace,monospace;font-size:12px;width:340px;max-height:86vh;overflow-y:auto;box-shadow:0 0 0 1px #1e3a5f,0 24px 64px rgba(0,0,0,0.9);';
            const hdr=document.createElement('div');
            hdr.style.cssText='display:flex;justify-content:space-between;align-items:center;margin-bottom:14px;';
            ...
            expBtn.textContent='📋 Kopiuj ustawienia (JSON)';
            expBtn.style.cssText='margin-top:16px;width:100%;padding:9px;background:#1e3a5f;border:1px solid #60a5fa;color:#60a5fa;border-radius:8px;cursor:pointer;font-size:11px;font-family:ui-monospace,monospace;';
            expBtn.onmouseenter=()=>expBtn.style.background='#2d4f7c';
            expBtn.onmouseleave=()=>expBtn.style.background='#1e3a5f';
            expBtn.onclick=()=>{
                const preset={az:+_az.toFixed(0),el:+_el.toFixed(0),int:+_int.toFixed(2),temp:+_temp.toFixed(0),hemi:+_hemi.toFixed(2),rim:+_rim.toFixed(2),rimAz:+_rimAz.toFixed(0),shOp:+_shOp.toFixed(2),shBlur:+_shBlur,blOp:+_blOp.toFixed(2)};
                const txt=JSON.stringify(preset);
                if(navigator.clipboard) navigator.clipboard.writeText(txt).then(()=>{ expBtn.textContent='✓ Skopiowano!'; setTimeout(()=>expBtn.textContent='📋 Kopiuj ustawienia (JSON)',2000); });
                else { const ta=document.createElement('textarea'); ta.value=txt; document.body.appendChild(ta); ta.select(); document.execCommand('copy'); document.body.removeChild(ta); expBtn.textContent='✓ Skopiowano!'; setTimeout(()=>expBtn.textContent='📋 Kopiuj ustawienia (JSON)',2000); }
            };
            // Eksponuj setter dla statycznego panelu w index.html
            window._rglSetLight = function(key, val) {
                val = parseFloat(val);
                if(key==='az'){_az=val;_updMain();}
                else if(key==='el'){_el=val;_updMain();}
                else if(key==='int'){_int=val;_updMain();}
                else if(key==='temp'){_temp=val;_updMain();}
                else if(key==='hemi'){_hemi=val;if(_dbgHemi)_dbgHemi.intensity=val;}
                else if(key==='rim'){_rim=val;_updRim();}
                else if(key==='rimAz'){_rimAz=val;_updRim();}
                else if(key==='shOp'){_shOp=val;_updSh();}
                else if(key==='shBlur'){_shBlur=val;_updSh();}
                else if(key==='blOp'){_blOp=val;_updSh();}
            };
            window._rglGetLight = function() {
                return {az:_az,el:_el,int:_int,temp:_temp,hemi:_hemi,rim:_rim,rimAz:_rimAz,shOp:_shOp,shBlur:_shBlur,blOp:_blOp};
            };
            window._rglSetMat = function(key, val) {
                val = parseFloat(val);
                if (key === 'roughness') {
                    SHELF3D_MAT_ROUGHNESS = val;
                    scene.traverse(o => { if (o.isMesh && o.material && o.material.isMeshPhysicalMaterial) { o.material.roughness = val; o.material.needsUpdate = true; } });
                } else if (key === 'metalness') {
                    scene.traverse(o => { if (o.isMesh && o.material && o.material.isMeshPhysicalMaterial) { o.material.metalness = val; o.material.needsUpdate = true; } });
                } else if (key === 'envInt') {
                    SHELF3D_MAT_ENV = val;
            ...
            document.addEventListener('keydown',e=>{ if(e.ctrlKey&&e.shiftKey&&e.key==='L'){e.preventDefault();window._rglLightToggle();} });
        }
        ...
        // Synchronizacja cienia z geometrią półki (wywoływana po każdej zmianie)
        var _rglShadowFrame = 0;
        // Lerp targets dla płynnych cieni
        var _rglShadowTargetY = -5.0;
        var _rglShadowBlobW = 1, _rglShadowBlobD = 1, _rglShadowBlobX = 0, _rglShadowBlobZ = 0;
        var _rglShadowDirX = 0, _rglShadowDirZ = 0;
        var _rglShadowAnimating = false;

        // Ustaw cele (bez lerpa) — wywołaj po zbudowaniu półki
        window._rglSyncShadow = function() {
            if (!shelfGroup || !scene) return;
            var _bb = new THREE.Box3().setFromObject(shelfGroup);
            var _center = new THREE.Vector3(); _bb.getCenter(_center);
            _rglShadowTargetY = _bb.min.y - 0.02;
            _rglShadowBlobW  = (_bb.max.x - _bb.min.x) * 2.6;
            _rglShadowBlobD  = (_bb.max.z - _bb.min.z) * 2.4;
            _rglShadowBlobX  = _center.x;
            _rglShadowBlobZ  = _center.z;
            _rglShadowDirX   = _center.x;
            _rglShadowDirZ   = _center.z;
            _rglShadowAnimating = true; // włącz lerp
            if (_dbgDir1) {
                ...
                if (_dbgDir1.shadow && _dbgDir1.shadow.camera) {
                    ...
                    // Zacieśnij ramkę cieni do bryły (przekątna/2 + mały margines) — mapa 2048 px idzie na półkę,
                    // a nie na pustą podłogę → ostre cienie wewnętrzne niezależnie od wysokości. Podłogę robi __blobShadow__.
                    var _r = Math.sqrt(_sx*_sx + _sy*_sy + _sz*_sz) * 0.5 + 1.6;
        ...
        // Lerp cieni — wywoływane co klatkę w animate()
        function _rglLerpShadows() {
            if (!scene) return;
            var _sf = scene.getObjectByName('__shadowFloor__');
            var _bs = scene.getObjectByName('__blobShadow__');
            var _speed = 0.12; // prędkość lerpa (0=stop, 1=instant)
            if (_sf) {
                var _dy = _rglShadowTargetY - _sf.position.y;
                if (Math.abs(_dy) > 0.001) {
                    _sf.position.y += _dy * _speed;
                } else {
                    _sf.position.y = _rglShadowTargetY;
                    _rglShadowAnimating = false;
                }
            }
            if (_bs) {
                _bs.scale.x += (_rglShadowBlobW - _bs.scale.x) * _speed;
                _bs.scale.y += (_rglShadowBlobD - _bs.scale.y) * _speed;
                _bs.position.x += (_rglShadowBlobX - _bs.position.x) * _speed;
                _bs.position.z += (_rglShadowBlobZ - _bs.position.z) * _speed;
                _bs.position.y = _sf ? _sf.position.y - 0.01 : _rglShadowTargetY - 0.01;
            }
        }
        ...
        function animate() {
            ...
            // Wymuszaj pozycję kamery na mobile podczas animacji budowania półki.
            // controls.update() wewnętrznie nadpisuje camera.position ze swoich
            // sferycznych współrzędnych — lock ustawia je z powrotem PRZED update(),
            // więc OrbitControls "widzi" właściwą pozycję i ją zachowuje.
            if (_mobileBuildLockCam && window.innerWidth < 768 && camera && controls) {
                camera.position.copy(_mobileBuildLockCam.pos);
                controls.target.copy(_mobileBuildLockCam.target);
            }
            if(controls) controls.update();
            if(renderer && scene && camera) {
                renderer.render(scene, camera);
                /* Synchronizacja z loading screen — zamknij po pierwszej klatce 3D */
                if (typeof window.rglDismiss === 'function') window.rglDismiss();
            _rglShadowFrame++;
            // Co 4 klatki odśwież cel cienia (śledź animację budowania)
            if (_rglShadowFrame % 4 === 0 && shelfGroup) window._rglSyncShadow();
            // Lerp cieni co klatkę — płynne przejście
            _rglLerpShadows();
                if (typeof updateDimensionOverlay === 'function') updateDimensionOverlay();
...
function fitCameraToShelf() {
   ...
   function playShowcaseSwing(shelfType) {
       ...
       // Na mobile: brak showcase swing — kamera już ustawiona pod kątem, swing tylko dezorientuje
       ...
       // Obrót wstępny wyłączony (optymalizacja)
       if (shelfGroup) shelfGroup.rotation.y = 0;
   }
   // ── KONIEC SHOWCASE SWING ─────────────────────────────────────────────────

   function rebuildAndAnimateIn(config, withRotationAnimation = true) {
      // withRotationAnimation=false: tryb snapshot/wczytywanie — animacja natychmiastowa
      // _rglNoAnim: wymuszone natychmiastowe składanie na czas generowania zdjęcia (boki/półki/góra/przegródki od razu finalne)
      const _animDur = (typeof window !== 'undefined' && window._rglNoAnim) ? 0 : (withRotationAnimation ? 1 : 0);
      ...
      if (camera && controls) {
          ...
          const _cz = _isWide84 ? 17.6 : (_isMobile ? (_h >= 80 ? 20.4 : _h >= 60 ? 16.7 : 14.8) : (_h >= 80 ? 18.5 : 13));
          const _cx = _isWide84 ? 0.0001 : 7.4;
          const _cy = 0.5;
          gsap.to(camera.position, { x: _cx, y: _cy, z: _cz, duration: 0.4 * _animDur, ease: "power1.inOut", onUpdate: function() { controls.update(); } });
      ...
      return new Promise(resolve => {
            ...
            const masterTimeline = gsap.timeline({
                onComplete: () => {
                    // Zwolnij blokadę kamery — animacja budowania zakończona,
                    // od tej chwili onComplete może swobodnie korygować kamerę.
                    _mobileBuildLockCam = null;
                    ...
                    controls.update();
                    // Po zakończeniu animacji — ustaw kamerę prostą formułą (jak S11)
                    if (shelfGroup && camera && controls) {
                        const _isCustomEnd = (typeof SHELF_TYPES !== 'undefined') && SHELF_TYPES.some(t => t.id === shelfType && t.isCustomLayout);
                        if (_isCustomEnd) {
                            // Wzory/custom: po budowie ZOSTAW podwyższony kąt (jak standardowe), nie obracaj do przodu.
                            const _cwE = parseInt(config.width) || 44, _chE = parseInt(heightSelect && heightSelect.value) || 60;
                            const _ccE = shelf3dGetCamForSize(_cwE, _chE);
                            controls.target.set(_ccE.tx || 0, _ccE.ty || 0, _ccE.tz || 0);
                            const _dirE = new THREE.Vector3(_ccE.x - (_ccE.tx || 0), _ccE.y - (_ccE.ty || 0), _ccE.z - (_ccE.tz || 0)).normalize();
                            const _distE = (typeof _shelf3dCalcDist === 'function') ? _shelf3dCalcDist(_cwE, _chE, 1.40) : Math.hypot(_ccE.x, _ccE.y, _ccE.z);
                            const _npE = controls.target.clone().add(_dirE.multiplyScalar(_distE));
                            camera.position.set(_npE.x, _npE.y, _npE.z);
                            controls.update();
                        } else {
                            const _hEnd = parseInt(heightSelect && heightSelect.value) || 60;
                            const _wEnd = parseInt(config && config.width) || 0;
                            const _isMobEnd = window.innerWidth < 768;
                            const _isWide84End = _wEnd === 84;
                            const _czEnd = _isWide84End ? 17.6 : (_isMobEnd ? (_hEnd >= 80 ? 20.4 : _hEnd >= 60 ? 16.7 : 14.8) : (_hEnd >= 80 ? 18.5 : 13));
                            const _cxEnd = _isWide84End ? 0.0001 : 7.4;
                            controls.target.set(0, 0, 0);
                            camera.position.set(_cxEnd, 0.5, _czEnd);
                            controls.update();
                            if (_isWide84End) {
                                autoRotateEnabled = false;
                        ...
                        setTimeout(() => {
                            ...
                            if (shelfGroup) {
                                const arrowObjects = shelfGroup.children.filter(c =>
                                    c.isLine ||
                                    (c.isSprite && c.name && (c.name.startsWith('dimensionLabel_') || c.name.startsWith('dimensionCone_'))) ||
                                    (c.isMesh && c.name && (c.name.startsWith('mugHeightArrow_') || c.name.startsWith('mugWidthArrow_') || c.name.startsWith('dimensionCone_')))
                                );
                    ...
                    // Tekstury już zastosowane na starcie animacji (shelf3dReapplyMaterials przy scene.add)
                    // — drugie wywołanie powodowało błysk materiałów na końcu animacji, usunięte.
                    // Zastosuj oświetlenie dla aktualnej kombinacji kolorów (ważne przy wczytaniu z galerii)
                    if (typeof shelf3dApplyColorLightSmart === 'function') shelf3dApplyColorLightSmart();
                    // Override kąta światła zależny od rozmiaru (np. 80 cm gł.10/15) — po świetle koloru
                    if (typeof window._shelf3dApplyLightForSize === 'function') window._shelf3dApplyLightForSize(config.height, config.depth);
                    // Zaktualizuj punkt powrotu idle-resetu — przechwytuj pozycję kamery PO animacji budowania
                    setTimeout(() => {
                        if (camera && controls && shelfGroup) {
                            _idleResetCam = {
                                px: camera.position.x, py: camera.position.y, pz: camera.position.z,
                                tx: controls.target.x, ty: controls.target.y, tz: controls.target.z,
                                rotY: shelfGroup.rotation.y,
                                shelfY: shelfGroup.position.y
                            };
                            _shelf3dStartIdleTimer();
                        }
                    }, 600);
                    resolve();
            ...
            // Ustaw kamerę mobile — prosta formuła jak S11
            _mobileBuildLockCam = null;
            if (window.innerWidth < 768 && camera && controls) {
            ...
            if (shelfType === 'modular') {
                masterTimeline.add(() => {
                    ...
                    shelfGroup.traverse(function(_m){ if(_m.isMesh){ _m.castShadow=true; _m.receiveShadow=true; } });
                    const _sf=scene.getObjectByName("__shadowFloor__"); if(_sf && shelfGroup){ setTimeout(window._rglSyncShadow, 600); }

                    if (typeof shelf3dReapplyMaterials === 'function') shelf3dReapplyMaterials();

                    const assemblyStartTime = ">+0.1";
                    const dropDuration = 0.5 * _animDur;
                    const slideDuration = 0.4 * _animDur;
                    
                    if(lm_left && rm_right) masterTimeline.to([lm_left.position, rm_right.position], { y: (i) => finalPositions.get(i === 0 ? lm_left : rm_right).y, duration: dropDuration, ease: 'power2.out' }, assemblyStartTime);
                    if(lm_bottom && rm_bottom) masterTimeline.to([lm_bottom.position, rm_bottom.position], { x: (i) => finalPositions.get(i === 0 ? lm_bottom : rm_bottom).x, y: finalPositions.get(lm_bottom).y, duration: slideDuration, ease: 'power2.out' }, `${assemblyStartTime}+=0.1`);
                    if(lm_right && rm_left) masterTimeline.to([lm_right.position, rm_left.position], { y: (i) => finalPositions.get(i === 0 ? lm_right : rm_left).y, duration: dropDuration, ease: 'power2.out' }, `${assemblyStartTime}+=0.2`);

                    const allModuleShelves = [...lm_shelves, ...rm_shelves];
                    if(allModuleShelves.length > 0) masterTimeline.to(allModuleShelves.map(s => s.position), { y: (i) => finalPositions.get(allModuleShelves[i]).y, duration: dropDuration * 0.8, ease: 'power2.out', stagger: 0.05 }, `${assemblyStartTime}+=0.3`);
                    if(connectingShelves.length > 0) masterTimeline.to(connectingShelves.map(s => s.position), { y: (i) => finalPositions.get(connectingShelves[i]).y, duration: dropDuration * 0.8, ease: 'power2.out', stagger: 0.06 }, `${assemblyStartTime}+=0.5`);
                    if(lm_top && rm_top) masterTimeline.to([lm_top.position, rm_top.position], { y: (i) => finalPositions.get(i === 0 ? lm_top : rm_top).y, duration: dropDuration, ease: 'power2.out' }, `${assemblyStartTime}+=0.7`);
                ...
                masterTimeline.add(() => {
                    ...
                    // Skala dynamiczna — dostosuj do wysokości i szerokości półki
                    const _mugW = (parseFloat(config.width)  || 44) / 10;
                    const _mugH = (parseFloat(config.height) || 40) / 10;
                    const _mugMaxDim = Math.max(_mugW, _mugH);
                    let _mugScale;
                    if (window.innerWidth >= 768 && !window._rglNoAnim) {
                        // Desktop: dopasuj szerokość półki do okna 3D — też 84 cm wypełnia widok i nie ucina.
                        _mugScale = Math.min(Math.max(10.45 / _mugW, 1.0), 2.5); // 10.45 = docelowa szerokość w oknie (do strojenia)
                        if (parseInt(config.width) === 44) _mugScale *= 0.6; // 44 cm: −40% (desktop)
                    } else if (window.innerWidth < 768 && !window._rglNoAnim) {
                        // Mobile (żywy podgląd): dopasuj szerokość do widoku — też 60/84 wypełniają, a nie są malutkie.
                        _mugScale = Math.min(Math.max(8.5 / _mugW, 0.7), 2.0); // 8.5 = docelowa szerokość na mobile (do strojenia)
                        if (parseInt(config.width) === 44) _mugScale *= 0.85; // 44 cm: −15%
                    } else {
                        // Generowane zdjęcie — bez zmian (normalizacja po największym wymiarze)
                        _mugScale = Math.min(Math.max(4.84 / _mugMaxDim, 0.5), 1.3);
                    }
                    shelfGroup.scale.set(_mugScale, _mugScale, _mugScale);
                    ...
                    try {
                        ...
                        if (_levels.length > 0 && (config.addDividersTop || config.addDividersMiddle || config.addDividersBottom)) {
                            ...
                            // Przegródka pionowa: BoxGeometry(thickness, compartmentH, depth)
                            // Używamy shelf3dMakeMaterialForBoard od razu + userData.shelfMatInfo
                    ...
                    // Ustaw startowe pozycje (poza sceną) — na mobile mniejsze przesunięcia
                    const _isMobMug = window.innerWidth < 768;
                    shelfGroup.children.forEach(c => {
                        const f = finalPositions.get(c.name);
                        if (!f) return;
                        if (c === ms_left)        gsap.set(c.position, { x: f.x - (_isMobMug ? 4 : 9),  y: f.y,                          z: f.z });
                        else if (c === ms_right)  gsap.set(c.position, { x: f.x + (_isMobMug ? 4 : 9),  y: f.y,                          z: f.z });
                        else if (c === ms_top)    gsap.set(c.position, { x: f.x,                         y: f.y + (_isMobMug ? 5 : 12),   z: f.z });
                        else if (c === ms_bottom) gsap.set(c.position, { x: f.x,                         y: f.y - (_isMobMug ? 5 : 12),   z: f.z });
                        else                      gsap.set(c.position, { x: f.x,                         y: f.y + (_isMobMug ? 6 : 16),   z: f.z });
                    });

                    scene.add(shelfGroup); shelfGroup.traverse(function(_m){ if(_m.isMesh){ _m.castShadow=true; _m.receiveShadow=true; } }); const _sf=scene.getObjectByName("__shadowFloor__"); if(_sf && shelfGroup){ setTimeout(window._rglSyncShadow, 600); }

                    if (typeof shelf3dReapplyMaterials === 'function') shelf3dReapplyMaterials();
                    const startAt = ">+0.1";
                    const springEase = 'power2.out';

                    if (ms_bottom) {
                        masterTimeline.to(ms_bottom.position,
                            { y: finalPositions.get("bottomPanel").y, duration: 0.45 * _animDur, ease: 'power2.out' }, startAt);
                    }
                    if (ms_left && ms_right) {
                        masterTimeline.to([ms_left.position, ms_right.position],
                            { x: (i)=> finalPositions.get(i===0?"leftSide":"rightSide").x,
                              duration: 0.5 * _animDur, ease: 'power2.out' },
                            `${startAt}+=0.12`);
                    }
                    if (ms_internals.length > 0) {
                        masterTimeline.to(ms_internals.map(s => s.position),
                            { y: (i)=> finalPositions.get(ms_internals[i].name).y,
                              duration: 0.45 * _animDur, ease: springEase, stagger: 0.06 * _animDur },
                            `${startAt}+=0.3`);
                    }
                    if (ms_dividers.length > 0) {
                        masterTimeline.to(ms_dividers.map(d => d.position),
                            { y: (i)=> finalPositions.get(ms_dividers[i].name).y,
                              duration: 0.4 * _animDur, ease: springEase, stagger: 0.03 * _animDur },
                            `${startAt}+=0.45`);
                    }
                    if (ms_top) {
                        masterTimeline.to(ms_top.position,
                            { y: finalPositions.get("topPanel").y, duration: 0.45 * _animDur, ease: 'power2.out' },
                            `${startAt}+=0.6`);
                    }
                });
            } else {
                masterTimeline.add(() => {
                    ...
                    const startY = window.innerWidth < 768 ? 7 : 15;
                    ...
                    const _mobOff = 0;
                    shelfGroup.position.y = _mobOff;
                    let _patScale = Array.isArray(config.shelves) ? 1.1 * 0.8 : 1.1;
                    if (window.innerWidth >= 768 && !window._rglNoAnim) _patScale *= 1.2; // desktop: +20% dla wzorów/custom (mobile i zdjęcie bez zmian)
                    shelfGroup.scale.set(_patScale, _patScale, _patScale);
                    // Wzory/custom: wymuś kąt kamery jak przy standardowych półkach (z tabeli po rozmiarze; fallback = 44×60, podwyższony).
                    try {
                        const _cw = parseInt(config.width) || 44, _ch = parseInt(config.height) || 60;
                        const _cc = shelf3dGetCamForSize(_cw, _ch);
                        if (camera && controls) {
                            controls.target.set(_cc.tx || 0, _cc.ty || 0, _cc.tz || 0);
                            const _cdir = new THREE.Vector3(_cc.x - (_cc.tx || 0), _cc.y - (_cc.ty || 0), _cc.z - (_cc.tz || 0)).normalize();
                            const _cdist = (typeof _shelf3dCalcDist === 'function') ? _shelf3dCalcDist(_cw, _ch, 1.40) : Math.hypot(_cc.x, _cc.y, _cc.z);
                            const _cnp = controls.target.clone().add(_cdir.multiplyScalar(_cdist));
                            camera.position.set(_cnp.x, _cnp.y, _cnp.z);
                            controls.update();
                        }
                    } catch (_eCam) {}
                    autoRotateEnabled = false; // wzory/custom: nie obracaj po zbudowaniu — zostań na kącie budowy
                    scene.add(shelfGroup); shelfGroup.traverse(function(_m){ if(_m.isMesh){ _m.castShadow=true; _m.receiveShadow=true; } }); const _sf=scene.getObjectByName("__shadowFloor__"); if(_sf && shelfGroup){ setTimeout(window._rglSyncShadow, 600); }
                    if (typeof shelf3dReapplyMaterials === 'function') shelf3dReapplyMaterials();
                    const assemblyStartTime = ">+0.1";
                    const dropDuration = 0.55 * _animDur;
                    if (bottomPanel) { masterTimeline.to(bottomPanel.position, { y: finalPositions.get("bottomPanel").y, duration: dropDuration, ease: 'power2.out' }, assemblyStartTime); }
                    if (leftSide && rightSide) { masterTimeline.to([leftSide.position, rightSide.position], { y: finalPositions.get("leftSide").y, duration: dropDuration, ease: 'power2.out' }, `${assemblyStartTime}+=0.08`); }
                    if (internalShelves.length > 0) { masterTimeline.to(internalShelves.map(s => s.position), { y: (i) => finalPositions.get(internalShelves[i].name).y, duration: dropDuration * 0.75, ease: 'power2.out', stagger: 0.06 }, `${assemblyStartTime}+=0.2`); }
                    if (dividers.length > 0) {
                        ...
                        masterTimeline.to(dividers.map(d => d.position), { y: (i) => finalPositions.get(dividers[i].name).y, duration: dropDuration * 0.8, ease: 'power2.out', stagger: 0.055 }, `${assemblyStartTime}+=0.28`);
                        ...
                        masterTimeline.to(dividers.map(d => d.scale), { y: 1, duration: 0.4, stagger: 0.055, ease: 'power2.out' }, `${assemblyStartTime}+=0.33`);
                    ...
                    if (topPanel) { masterTimeline.to(topPanel.position, { y: finalPositions.get("topPanel").y, duration: dropDuration, ease: 'power2.out' }, `${assemblyStartTime}+=0.38`); }
        ...
        async function updatePreview(animateChanges = true) {
            ...
            if (shelfType === 'modular') {
                ...
                config = { width: getCurrentWidth(), height: heightSelect.value, depth: depthSelect.value, shelfCount: shelfCountSelect.value, shelfType: shelfType, noTopShelf: (SHELF_TYPES.find(t => t.id === shelfType && t.isCustomLayout) ? !!document.getElementById("cust-notop")?.checked : !!document.getElementById("noTopShelf")?.checked), noBottomShelf: (SHELF_TYPES.find(t => t.id === shelfType && t.isCustomLayout) ? !!document.getElementById("cust-nobot")?.checked : (shelfType === "standing" && !!document.getElementById("noBottomShelf")?.checked)), addDividersTop: shelfType === 'mug_shelf' && dividersTopCheckbox?.checked, addDividersMiddle: shelfType === 'mug_shelf' && dividersMiddleCheckbox?.checked, addDividersBottom: shelfType === 'mug_shelf' && dividersBottomCheckbox?.checked, customPositions: (typeof customShelfPositionEnabled !== 'undefined' && customShelfPositionEnabled && customShelfPositions.length > 0 && shelfType !== 'mug_shelf') ? [...customShelfPositions] : null,
            ...
            // Kamera NIE zmienia pozycji przy zmianie wymiarów — półka skaluje się w miejscu (jak S11)
            ...
            if (config.shelves && config.shelves.length > 0) {
                ...
                const _mkPanelMat = (w, h, d) => (typeof shelf3dMakeMaterialForBoard === 'function')
                    ? shelf3dMakeMaterialForBoard('shelf', w, h, d)
                    : new THREE.MeshStandardMaterial(originalPreviewMaterialParams);

                if (_topExists) {
                    if (!_tP) {
                        const _newTP = new THREE.Mesh(new THREE.BoxGeometry(_tW - 2*_tk, _tk, _tD), _mkPanelMat(_tW - 2*_tk, _tk, _tD));
                ...
                if (_botExists) {
                    if (!_bP) {
                        const _newBP = new THREE.Mesh(new THREE.BoxGeometry(_tW - 2*_tk, _tk, _tD), _mkPanelMat(_tW - 2*_tk, _tk, _tD));
            ...
            currentAnimationTimeline = gsap.timeline({
                onComplete: () => {
                    // Zaktualizuj idle-reset po płynnej zmianie wymiarów
                    setTimeout(() => {
                        if (camera && controls && shelfGroup) {
                            _idleResetCam = {
                                px: camera.position.x, py: camera.position.y, pz: camera.position.z,
            ...
            const animDuration = (typeof window !== 'undefined' && window._rglNoAnim) ? 0 : 0.5;
            ...
            if (shelfType === 'modular') {
                ...
                const animateModule = (module) => {
                    ...
                    for (let i = 0; i < newShelfCount; i++) {
                        ...
                        const _sW = targetModuleW - 2 * thickness, _sD = targetDepth;
                        const _shelfMat = (typeof shelf3dMakeMaterialForBoard === 'function')
                            ? shelf3dMakeMaterialForBoard('shelf', _sW, thickness, _sD)
                            : defaultMaterial.clone();
                        const shelf = new THREE.Mesh(shelfGeo, _shelfMat);
                        shelf.name = `internalShelf_${i}`;
                        shelf.userData.shelfMatInfo = { type: 'shelf', w: _sW, h: thickness, d: _sD };
                ...
                for (let i = 0; i < newConnectingShelvesYPositions.length - 1; i++) {
                    ...
                    const _cMat = (typeof shelf3dMakeMaterialForBoard === 'function')
                        ? shelf3dMakeMaterialForBoard('shelf', targetConnectW, thickness, targetDepth)
                        : defaultMaterial.clone();
                    const shelf = new THREE.Mesh(shelfGeo, _cMat);
                    shelf.name = `connecting_shelf_${i}`;
                    shelf.userData.shelfMatInfo = { type: 'shelf', w: targetConnectW, h: thickness, d: targetDepth };
                ...
                const _mkBoardMat = (type, w, h, d) => (typeof shelf3dMakeMaterialForBoard === 'function')
                    ? shelf3dMakeMaterialForBoard(type, w, h, d)
                    : new THREE.MeshStandardMaterial(originalPreviewMaterialParams);
            ...
            if (!leftSide || !rightSide) {
                ...
                // Półka na kubki: zaktualizuj skalę grupy przy zmianie szerokości/wysokości
                if (shelfType === 'mug_shelf') {
                    const _newMugMaxDim = Math.max(targetWidth, targetHeight);
                    let _newMugScale;
                    if (window.innerWidth >= 768 && !window._rglNoAnim) {
                        _newMugScale = Math.min(Math.max(10.45 / targetWidth, 1.0), 2.5); // desktop: dopasuj szerokość do okna 3D (też 84 cm)
                        if (parseInt(config.width) === 44) _newMugScale *= 0.6; // 44 cm: −40% (desktop)
                    } else if (window.innerWidth < 768 && !window._rglNoAnim) {
                        _newMugScale = Math.min(Math.max(8.5 / targetWidth, 0.7), 2.0); // mobile: dopasuj szerokość do widoku (też 60/84)
                        if (parseInt(config.width) === 44) _newMugScale *= 0.85; // 44 cm: −15%
                    } else {
                        _newMugScale = Math.min(Math.max(4.84 / _newMugMaxDim, 0.5), 1.3); // generowane zdjęcie — bez zmian
                    }
                    currentAnimationTimeline.to(shelfGroup.scale, { x: _newMugScale, y: _newMugScale, z: _newMugScale, duration: animDuration, ease: 'power2.inOut' }, 0);
                    // Zaktualizuj idle-reset po zmianie skali
                ...
                currentAnimationTimeline.to(leftSide.scale, { y: targetHeight / leftSide.geometry.parameters.height, z: targetDepth / leftSide.geometry.parameters.depth, duration: animDuration, ease: 'power2.inOut' }, 0);
                currentAnimationTimeline.to(rightSide.scale, { y: targetHeight / rightSide.geometry.parameters.height, z: targetDepth / rightSide.geometry.parameters.depth, duration: animDuration, ease: 'power2.inOut' }, 0);
                currentAnimationTimeline.to(leftSide.position, { x: -targetWidth / 2 + thickness / 2, duration: animDuration, ease: 'power2.inOut' }, 0);
                currentAnimationTimeline.to(rightSide.position, { x: targetWidth / 2 - thickness / 2, duration: animDuration, ease: 'power2.inOut' }, 0);
                ...
                if (topExists) {
                    if (!topPanel) {
                        topPanel = new THREE.Mesh(new THREE.BoxGeometry(targetWidth - 2 * thickness, thickness, targetDepth), _mkBoardMat('shelf', targetWidth - 2 * thickness, thickness, targetDepth));
                        topPanel.name = "topPanel"; topPanel.scale.set(1,0.01,1); shelfGroup.add(topPanel);
                    }
                    currentAnimationTimeline.to(topPanel.scale, { x: (targetWidth - 2*thickness) / topPanel.geometry.parameters.width, y: 1, z: targetDepth / topPanel.geometry.parameters.depth, duration: animDuration, ease: 'power2.inOut' }, 0);
                    currentAnimationTimeline.to(topPanel.position, { y: targetHeight / 2 - thickness / 2, duration: animDuration, ease: 'power2.inOut' }, 0);
                } else if (topPanel) {
                     currentAnimationTimeline.to(topPanel.scale, { y: 0.01, duration: animDuration / 2, ease: 'power2.inOut', onComplete: () => shelfGroup.remove(topPanel) }, 0);
                }

                const bottomExists = !config.noBottomShelf;
                if (bottomExists) {
                    if (!bottomPanel) {
                        bottomPanel = new THREE.Mesh(new THREE.BoxGeometry(targetWidth - 2 * thickness, thickness, targetDepth), _mkBoardMat('shelf', targetWidth - 2 * thickness, thickness, targetDepth));
                        bottomPanel.name = "bottomPanel"; bottomPanel.scale.set(1,0.01,1); shelfGroup.add(bottomPanel);
                    }
                    currentAnimationTimeline.to(bottomPanel.scale, { x: (targetWidth - 2*thickness) / bottomPanel.geometry.parameters.width, y:1, z: targetDepth / bottomPanel.geometry.parameters.depth, duration: animDuration, ease: 'power2.inOut' }, 0);
                    currentAnimationTimeline.to(bottomPanel.position, { y: -targetHeight / 2 + thickness / 2, duration: animDuration, ease: 'power2.inOut' }, 0);
                } else if (bottomPanel) {
                    currentAnimationTimeline.to(bottomPanel.scale, { y: 0.01, duration: animDuration / 2, ease: 'power2.inOut', onComplete: () => shelfGroup.remove(bottomPanel) }, 0);
                }
                ...
                if (shelvesToRemove.length > 0) {
                     currentAnimationTimeline.to(shelvesToRemove.map(s => s.scale), { x: 0.01, duration: animDuration / 2, ease: 'power2.inOut', onComplete: () => {
                ...
                if (shelfType === 'mug_shelf' && config.height === '60') {
                    ...
                    for (let i = 0; i < targetShelfCount; i++) {
                        ...
                        if (!shelf) {
                             shelf = new THREE.Mesh(new THREE.BoxGeometry(targetWidth - 2 * thickness, thickness, targetDepth), _mkBoardMat('shelf', targetWidth - 2 * thickness, thickness, targetDepth));
                             shelf.name = shelfName; shelf.position.y = targetY; shelf.scale.set(0.01, 1, 1); shelf.castShadow = true; shelf.receiveShadow = true; shelfGroup.add(shelf);
                             currentAnimationTimeline.to(shelf.scale, { x: 1, duration: animDuration, ease: 'power2.inOut', delay: (i * 0.1) }, 0);
                        } else {
                             currentAnimationTimeline.to(shelf.position, { y: targetY, duration: animDuration, ease: 'power2.inOut' }, 0);
                             currentAnimationTimeline.to(shelf.scale, { x: (targetWidth - 2 * thickness) / shelf.geometry.parameters.width, z: targetDepth / shelf.geometry.parameters.depth, duration: animDuration, ease: 'power2.inOut' }, 0);
                        }
                    ...
                    // Helper: filtr wszystkich elementów strzałek (linie + sprity etykiet + sprity stożków)
                    const _isArrowObj = c =>
                        c.isLine ||
                        (c.isSprite && c.name && (c.name.startsWith('dimensionLabel_') || c.name.startsWith('dimensionCone_'))) ||
                        (c.isMesh  && c.name && c.name.startsWith('dimensionCone_'));

                    if (useCustomPos) {
                        // Ukryj stare strzałki natychmiast (animacja trwa)
                        shelfGroup.children.filter(_isArrowObj).forEach(c => {
                            if (c.material) { c.material.transparent = true; c.material.opacity = 0; }
                        });

                        setTimeout(() => {
                            // Usuń stare elementy strzałek
                            const toRemove = shelfGroup.children.filter(_isArrowObj);
                            toRemove.forEach(c => { if (c.geometry) c.geometry.dispose(); if (c.material) { if (c.material.map) c.material.map.dispose(); c.material.dispose(); } shelfGroup.remove(c); });
                            ...
                            if (internalShelves.length > 0 && shelfType !== 'mug_shelf' && typeof addShelfDimensionArrows === 'function') {
                                ...
                                const newArrows = shelfGroup.children.filter(_isArrowObj);
                                newArrows.forEach(o => { if (o.material) { o.material.transparent = true; o.material.opacity = 0; } });
                                if (newArrows.length > 0) gsap.to(newArrows.map(o=>o.material).filter(Boolean), { opacity:1, duration:0.45, ease:'power2.out', stagger:0.04 });
                            }
            ...
            if (shelfType === 'mug_shelf') {
                ...
                currentAnimationTimeline.call(() => {
                    ...
                    // Wyczyść stare wymiary SVG tej grupy (zawsze, niezależnie od przegródek)
                    if (Array.isArray(_dimSegments)) {
                        _dimSegments = _dimSegments.filter(s => s.parent !== shelfGroup);
                    }
                    ...
                    const createDividers = (levelData, levelName) => {
                        ...
                        // Wydajność: jeden materiał + jedna geometria na poziom (zamiast per przegródka) — mniej lagów.
                        // Przegródki danego poziomu powstają i znikają razem, więc współdzielenie jest bezpieczne.
                        const _divMatShared = _mkDivM();
                        if (widthNum == 84) {
                            const numPhysicalDividers = 3;
                            const singleCompartmentWidth = 1.25;
                            ...
                            for (let i = 0; i < numPhysicalDividers; i++) {
                                ...
                                const m = new THREE.Mesh(geo, _divMatShared);
        ...
        function createSingleModule(config, material) {
            ...
            const horizInfo = { type: 'shelf', w: width - 2 * thickness, h: thickness, d: depth };
            const sideInfo  = { type: 'side',  w: depth, h: height, d: depth };
            ...
            if (!noBottomShelf) {
                ...
                bottomPanel.userData.shelfMatInfo = { ...horizInfo };
            ...
            if (!noTopShelf) {
                ...
                topPanel.userData.shelfMatInfo = { ...horizInfo };
            ...
            leftSide.userData.shelfMatInfo = { ...sideInfo };
            ...
            rightSide.userData.shelfMatInfo = { ...sideInfo };
            ...
            if (shelfCount > 0) {
                ...
                for(let i = 0; i < shelfCount; i++) {
                    ...
                    const shelf = new THREE.Mesh(innerShelfGeo, material.clone());
                    const yPos = startY + (i + 1) * gap + (i + 0.5) * thickness;
                    shelf.position.y = yPos;
                    shelf.name = `internalShelf_${i}`;
                    shelf.userData.shelfMatInfo = { ...horizInfo };
        ...
        function buildModularShelf(config) { 
            ...
            const connectHorizInfo = { type: 'shelf', w: connectingShelfWidth, h: thickness, d: moduleConfig.depth };
        ...
        function buildShelfModel(config) { const group = new THREE.Group(); const { width: widthNum, height: heightVal, depth: depthVal, shelfCount: shelfCountVal, shelfType, noTopShelf, noBottomShelf, addDividersTop, addDividersMiddle, addDividersBottom } = config; if (!widthNum || !heightVal || !depthVal || shelfCountVal === undefined) return group; const defaultMaterial = new THREE.MeshStandardMaterial(originalPreviewMaterialParams); const mkMat = () => defaultMaterial.clone(); const thickness = 0.18; const width = widthNum / 10; const height = parseFloat(heightVal) / 10; const depth = parseFloat(depthVal) / 10; const shelfCount = parseInt(shelfCountVal); if (isNaN(width) || isNaN(height) || isNaN(depth) || width <= 0 || height <= 0 || depth <= 0) return group; let topPanel, bottomPanel; if (!noBottomShelf) { const bottomGeometry = new THREE.BoxGeometry(width - 2 * thickness, thickness, depth); bottomPanel = new THREE.Mesh(bottomGeometry, mkMat()); bottomPanel.position.set(0, -height / 2 + thickness / 2, 0); bottomPanel.name = "bottomPanel"; bottomPanel.castShadow = true; bottomPanel.receiveShadow = true; group.add(bottomPanel); } if (!noTopShelf) { const topGeometry = new THREE.BoxGeometry(width - 2 * thickness, thickness, depth); topPanel = new THREE.Mesh(topGeometry, mkMat()); topPanel.position.set(0, height / 2 - thickness / 2, 0); topPanel.name = "topPanel"; topPanel.castShadow = true; topPanel.receiveShadow = true; group.add(topPanel); } const sideGeometry = new THREE.BoxGeometry(thickness, height, depth); const leftSideMesh = new THREE.Mesh(sideGeometry, mkMat()); leftSideMesh.position.set(-width / 2 + thickness / 2, 0, 0); leftSideMesh.name = "leftSide"; leftSideMesh.castShadow = true; leftSideMesh.receiveShadow = true; group.add(leftSideMesh); const rightSideMesh = new THREE.Mesh(sideGeometry, mkMat()); rightSideMesh.position.set(width / 2 - thickness / 2, 0, 0); rightSideMesh.name = "rightSide"; rightSideMesh.castShadow = true; rightSideMesh.receiveShadow = true; group.add(rightSideMesh); const internalShelves = []; if (!isNaN(shelfCount) && shelfCount > 0) { if (shelfType === 'mug_shelf' && heightVal === '60' && topPanel) { const fixedGap = 1.09; const topPanelBottomY = topPanel.position.y - thickness / 2; let lastShelfY = topPanelBottomY; const mugShelfCount60 = 3; for (let i = 0; i < mugShelfCount60; i++) { const shelfY = lastShelfY - fixedGap - thickness / 2; const shelfGeometry = new THREE.BoxGeometry(width - 2 * thickness, thickness, depth); const shelfMesh = new THREE.Mesh(shelfGeometry, mkMat()); shelfMesh.position.set(0, shelfY, 0); shelfMesh.name = `internalShelf_${i}`; shelfMesh.castShadow = true; shelfMesh.receiveShadow = true; group.add(shelfMesh); internalShelves.push(shelfMesh); lastShelfY = shelfY - thickness / 2; } } else { const topExists = !!topPanel; const bottomExists = !!bottomPanel; const topThicknessActual = topExists ? thickness : 0; const bottomThicknessActual = bottomExists ? thickness : 0; const totalAvailableHeight = height - topThicknessActual - bottomThicknessActual; if (totalAvailableHeight >= shelfCount * thickness && (shelfCount + 1) > 0) { const useCustom = config.customPositions && config.customPositions.length === shelfCount; if (useCustom) { const bottomY = -height / 2 + thickness; for (let i = 0; i < shelfCount; i++) { const posFromBottom = config.customPositions[i] / 10; const shelfY = bottomY + posFromBottom; const shelfGeometry = new THREE.BoxGeometry(width - 2 * thickness, thickness, depth); const shelfMesh = new THREE.Mesh(shelfGeometry, mkMat()); shelfMesh.position.set(0, shelfY, 0); shelfMesh.name = `internalShelf_${i}`; group.add(shelfMesh); internalShelves.push(shelfMesh); } } else { const gap = (height - 2*thickness - shelfCount * thickness) / (shelfCount + 1); let startY = -height / 2 + thickness; for (let i = 1; i <= shelfCount; i++) { const shelfY = startY + i * gap + (i - 0.5) * thickness; const shelfGeometry = new THREE.BoxGeometry(width - 2 * thickness, thickness, depth); const shelfMesh = new THREE.Mesh(shelfGeometry, mkMat()); shelfMesh.position.set(0, shelfY, 0); shelfMesh.name = `internalShelf_${i-1}`; group.add(shelfMesh); internalShelves.push(shelfMesh); } } } } } if (config.customPositions && internalShelves.length > 0) { if (typeof addShelfDimensionArrows === 'function') addShelfDimensionArrows(group, internalShelves, bottomPanel, topPanel, thickness, width, height, depth); } if (shelfType === 'mug_shelf' && topPanel && bottomPanel && internalShelves.length > 0) { if (addDividersTop || addDividersMiddle || addDividersBottom) { const innerWidthForDividers = width - 2 * thickness; const dividerThicknessThreeJs = thickness; let levels = []; internalShelves.sort((a, b) => a.position.y - b.position.y); if (heightVal === '60' && internalShelves.length === 3) { levels = [ { top: topPanel.position.y - thickness / 2, bottom: internalShelves[2].position.y + thickness/2 }, { top: internalShelves[2].position.y - thickness/2, bottom: internalShelves[1].position.y + thickness/2 }, { top: internalShelves[1].position.y - thickness/2, bottom: internalShelves[0].position.y + thickness/2 }, { top: internalShelves[0].position.y - thickness/2, bottom: bottomPanel.position.y + thickness / 2 } ]; } else if (heightVal === '40' && internalShelves.length === 2) { levels = [ { top: topPanel.position.y - thickness / 2, bottom: internalShelves[1].position.y + thickness/2 }, { top: internalShelves[1].position.y - thickness/2, bottom: internalShelves[0].position.y + thickness/2 }, { top: internalShelves[0].position.y - thickness/2, bottom: bottomPanel.position.y + thickness / 2 } ]; } const drawDividersForLevel = (levelData) => { const levelH = Math.max(0.01, levelData.top - levelData.bottom); if (levelH <= 0.01) return; const yCenter = levelData.bottom + levelH / 2; const _mkDivMat2 = () => { const _m = (typeof shelf3dMakeMaterialForBoard === 'function') ? shelf3dMakeMaterialForBoard('shelf', levelH, dividerThicknessThreeJs, depth) : mkMat(); return _m; }; const _divMatInfo = { type: 'shelf', w: levelH, h: dividerThicknessThreeJs, d: depth }; if (widthNum == 84) { const numPhysicalDividers = 3; const singleCompartmentWidth = 1.25; let currentXPos = -innerWidthForDividers / 2; for (let i = 0; i < numPhysicalDividers; i++) { currentXPos += singleCompartmentWidth; const dividerXCenter = currentXPos + (dividerThicknessThreeJs / 2); const dividerMesh = new THREE.Mesh(new THREE.BoxGeometry(dividerThicknessThreeJs, levelH, depth), _mkDivMat2()); dividerMesh.userData.shelfMatInfo = { ..._divMatInfo }; dividerMesh.position.set(dividerXCenter, yCenter, 0); dividerMesh.name = `divider_h84_${i}`; group.add(dividerMesh); currentXPos += dividerThicknessThreeJs; } } else { const numDividersOriginal = (widthNum == 60) ? 3 : ((widthNum == 44) ? 2 : 0); if (numDividersOriginal > 0) { const dividerSpacing = innerWidthForDividers / (numDividersOriginal + 1); for (let i = 1; i <= numDividersOriginal; i++) { const xPos = (-innerWidthForDividers / 2) + i * dividerSpacing; const dividerMesh = new THREE.Mesh(new THREE.BoxGeometry(dividerThicknessThreeJs, levelH, depth), _mkDivMat2()); dividerMesh.userData.shelfMatInfo = { ..._divMatInfo }; dividerMesh.position.set(xPos, yCenter, 0); dividerMesh.name = `divider_${i}`; group.add(dividerMesh); } } } }; if (addDividersTop && levels[0]) drawDividersForLevel(levels[0]); if (addDividersMiddle && levels[1]) drawDividersForLevel(levels[1]); if (addDividersBottom && levels[2]) drawDividersForLevel(levels[2]); // --- Wymiary przegródek kubkowych ---
            ...
            return group; }
        function generate3dSnapshotFromCurrentModel(forceFresh = false) {
            return new Promise((resolve, reject) => {
                // Zawsze renderuj z domyślnej pozycji kamery (nie aktualnej — użytkownik mógł obrócić/zbliżyć)
                // forceFresh=true: pomiń live renderer (używa fresh model bez animacji — dla podglądu projektu)
                if (!forceFresh && renderer && scene && camera && controls) {
                    try {
                        // Zachowaj aktualną pozycję
                        const _savedPos = camera.position.clone();
                        const _savedTgt = controls.target.clone();
                        const _curType = shelfTypeSelect ? shelfTypeSelect.value : '';

                        // Ustaw kamerę — dynamicznie na podstawie bounding box (działa dla każdej szerokości)
                        if (shelfGroup) {
                            // Licz bounding box tylko z geometrii półki — pomijaj strzałki wymiarów,
                            // które wystają poza obrys i powodują zbyt duże oddalenie kamery w snapshocie
                            const _box = new THREE.Box3();
                            shelfGroup.traverse(child => {
                                if (!child.isMesh && !child.isLine) return;
                                if (child.isLine) return; // linie = strzałki wymiarów
                                const n = child.name || '';
                                if (n.startsWith('dimensionLabel_') || n.startsWith('mugHeightArrow_') ||
                                    n.startsWith('mugWidthArrow_') || n.startsWith('dimensionCone_')) return;
                                if (child.isSprite) return;
                                child.updateMatrixWorld(true);
                                const childBox = new THREE.Box3().setFromObject(child);
                                _box.union(childBox);
                            });
                            if (_box.isEmpty()) _box.setFromObject(shelfGroup);
                            const _bSize = _box.getSize(new THREE.Vector3());
                            const _bCtr  = _box.getCenter(new THREE.Vector3());
                            const _fovV  = camera.fov * Math.PI / 180;
                            const _fovH  = 2 * Math.atan(Math.tan(_fovV / 2) * camera.aspect);
                            const _pad   = _curType === 'mug_shelf' ? 2.6 : 2.05;
                            const _dV    = (_bSize.y / 2) / Math.tan(_fovV / 2) * _pad;
                            const _dH    = (_bSize.x / 2) / Math.tan(_fovH / 2) * _pad;
                            // dla mug_shelf uwzględnij też głębokość (oś Z) — półka jest płaska i bez tego kamera jest za blisko
                            const _dZ    = _curType === 'mug_shelf' ? (_bSize.z / 2) / Math.tan(_fovH / 2) * _pad : 0;
                            const _dist  = Math.max(_dV, _dH, _dZ);
                            // Wzorcowy kierunek kamery: dla mug_shelf stały kąt, dla innych — z tabeli 44cm
                            let _refDir;
                            if (_curType === 'mug_shelf') {
                                _refDir = new THREE.Vector3(4.9693, 2.9140, 7.4658).normalize();
                            } else {
                                const _sh = parseInt(heightSelect && heightSelect.value) || 60;
                                const _sc = shelf3dGetCamForSize(44, _sh);
                                _refDir = new THREE.Vector3(_sc.x - (_sc.tx||0), _sc.y - (_sc.ty||0), _sc.z - (_sc.tz||0)).normalize();
                            }
                            const _np = _bCtr.clone().add(_refDir.multiplyScalar(_dist));
                            camera.position.copy(_np);
                            controls.target.copy(_bCtr);
                        } else {
                            // Fallback gdy shelfGroup niedostępny
                            if (_curType === 'mug_shelf') {
                                camera.position.set(9.2081, 5.3996, 13.8341);
                                controls.target.set(0, 0, 0);
                            } else {
                                const _sw = parseInt(getCurrentWidth && getCurrentWidth()) || 44;
                                const _sh = parseInt(heightSelect && heightSelect.value) || 60;
                                const _sc = shelf3dGetCamForSize(_sw, _sh);
                                camera.position.set(_sc.x, _sc.y, _sc.z);
                                controls.target.set(_sc.tx || 0, _sc.ty || 0, _sc.tz || 0);
                            }
                        }
                        ...
                        let _snap;
                        try {
                            const _liveSegs = (Array.isArray(_dimSegments) ? _dimSegments : []).filter(s => {
                                let p = s.parent;
                                while (p) { if (p === shelfGroup) return true; p = p.parent; }
                                return false;
                            });
                            const _cw = renderer.domElement.width, _ch = renderer.domElement.height;
                            ...
                            _snap = _cnv.toDataURL('image/png');
                        } catch (_eLive) {
                        ...
                        if (_snap && _snap.length > 1000) return resolve(_snap);
                    } catch(e) {}
                }
                const shelfType = shelfTypeSelect ? shelfTypeSelect.value : '';
                let freshConfig;
                ...
                // Aplikuj poprawne materiały (kolor/tekstura) — identycznie jak shelf3dReapplyMaterials
                if (typeof shelf3dMakeMaterialForBoard === 'function') {
                    if (typeof shelf3dUpdateFactory === 'function') shelf3dUpdateFactory();
                ...
                snapshotScene.background = new THREE.Color(0xf0f0ee);
                const snapshotCamera = new THREE.PerspectiveCamera(45, 1, 0.1, 1000);

                // Kopiuj intensywności z żywej sceny (shelf3dApplyColorLightSmart),
                // ale zachowaj oryginalne pozycje świateł — pozycja z az/el działa tylko w live scene
                const _hemiInt = (_dbgHemi && _dbgHemi.intensity != null) ? _dbgHemi.intensity : 0.50;
                const _dir1Int = (_dbgDir1 && _dbgDir1.intensity != null) ? _dbgDir1.intensity : 2.00;
                const _dir2Int = (_dbgDir2 && _dbgDir2.intensity != null) ? _dbgDir2.intensity : 0.55;
                const _liveRim = scene ? scene.getObjectByName('__rimLight__') : null;
                const _rimInt  = (_liveRim && _liveRim.intensity != null) ? _liveRim.intensity : 0.60;
                ...
                // Przechwyć segmenty wymiarowe tej bryły do wypalenia na snapshocie
                const _snapDimSegs = (Array.isArray(_dimSegments) ? _dimSegments : []).filter(s => {
                    let p = s.parent;
                    while (p) { if (p === freshGroup) return true; p = p.parent; }
                    return false;
                });
                // Usuń je z globalnej listy, żeby nie kolidowały z overlayem live
                if (Array.isArray(_dimSegments) && _snapDimSegs.length) {
                    const _set = new Set(_snapDimSegs);
                    _dimSegments = _dimSegments.filter(s => !_set.has(s));
                }

                // Bounding box TYLKO geometrii półki (bez strzałek) — centrum i rozmiar do pozycji kamery
                const meshOnly = new THREE.Box3();
                freshGroup.traverse(ch => {
                    if (ch.isMesh && !ch.isSprite && !(ch.name && (ch.name.startsWith('dimensionLabel_') || ch.name.startsWith('dimensionCone_') || ch.name.startsWith('mugHeightArrow_') || ch.name.startsWith('mugWidthArrow_')))) {
                        meshOnly.expandByObject(ch);
                    }
                });
                const shelfCenter = meshOnly.getCenter(new THREE.Vector3());
                const shelfSize   = meshOnly.getSize(new THREE.Vector3());
                const maxDim = Math.max(shelfSize.x, shelfSize.y, shelfSize.z);
                const fov = snapshotCamera.fov * (Math.PI / 180);
                let distance = maxDim / (2 * Math.tan(fov / 2));
                distance *= 0.85; // półka wypełnia kadr
                const finalDistance = Math.max(distance, 1.5);

                // Kąt identyczny jak w live scene — używa tej samej tabeli kamery co podgląd 3D
                const _snapH = parseInt(freshConfig.height) || 60;
                const _snapCamData = (typeof shelf3dGetCamForSize === 'function')
                    ? shelf3dGetCamForSize(44, _snapH)
                    : { x: 4.455, y: 2.7246, z: 7.1042, tx: 0, ty: 0, tz: 0 };
                const viewVector = new THREE.Vector3(
                    _snapCamData.x - (_snapCamData.tx || 0),
                    _snapCamData.y - (_snapCamData.ty || 0),
                    _snapCamData.z - (_snapCamData.tz || 0)
                ).normalize();
                snapshotCamera.position.copy(viewVector).multiplyScalar(finalDistance).add(shelfCenter);
                snapshotCamera.lookAt(shelfCenter);

                const _SNAP_SIZE = 1800;
                const snapshotRenderer = new THREE.WebGLRenderer({ antialias: true, alpha: false, preserveDrawingBuffer: true });
                snapshotRenderer.setClearColor(0xf0f0ee, 1);
                ...
                setTimeout(() => {
                    try {
                        ...
                        // Wypal wymiary (overlay SVG nie trafia do toDataURL) na kopii 2D
                        let dataUrl;
                        try {
                            const _cnv = document.createElement('canvas');
                            ...
                            dataUrl = _cnv.toDataURL('image/png');
                        } catch (_eComp) {
                            dataUrl = snapshotRenderer.domElement.toDataURL('image/png');
                        }
        ...
        function reset3DView() { if (controls && camera) { const _isMob = window.innerWidth < 768; const _hh = parseInt(heightSelect && heightSelect.value) || 60; const _ww = parseInt(widthSelect && widthSelect.value) || 44; const _isWide84r = _isMob && _ww === 84; const _czR = _isWide84r ? 17.6 : (_isMob ? (_hh >= 80 ? 20.4 : _hh >= 60 ? 16.7 : 14.8) : (_hh >= 80 ? 18.5 : 13)); const _cxR = _isWide84r ? 0.0001 : 7.4; camera.position.set(_cxR, 0.5, _czR); controls.target.set(0, 0, 0); controls.update(); setActiveMobileIcon(null); if (shelfTypeSelect && shelfTypeSelect.value !== 'mug_shelf' && shelfTypeSelect.value !== 'modular') { autoRotateEnabled = false; } } }
        function computePriceDetailed() {
            const shelfType = shelfTypeSelect.value;
            // ── isCustomLayout: wzór z edytora — jawna matryca cen per szerokość × głębokość ──
            const _customLayoutType = SHELF_TYPES.find(t => t.id === shelfType && t.isCustomLayout);
            if (_customLayoutType) {
                const _ct = _customLayoutType;
                const _cp = (typeof customPrices !== 'undefined') ? customPrices[shelfType] : undefined;
                if (_cp === undefined || _cp === null) return null;
                const _selW = String(parseInt(getCurrentWidth()) || _ct.width);
                const _selD = String(parseInt(depthSelect.value) || _ct.depth || 10);
                // Format obiektowy: { prices: { szer: { gł: cena } }, noTop, noBottom }
                if (typeof _cp === 'object') {
                    let _price = null;
                    const _pw = _cp.prices && _cp.prices[_selW];
                    if (_pw) _price = (_pw[_selD] != null) ? _pw[_selD] : (_pw['10'] != null ? _pw['10'] : null);
                    // awaryjnie: pierwsza dostępna cena lub base
                    if (_price == null && _cp.prices) {
                        const _firstW = Object.keys(_cp.prices)[0];
                        const _firstD = _firstW && Object.keys(_cp.prices[_firstW])[0];
                        if (_firstW && _firstD) _price = _cp.prices[_firstW][_firstD];
                    }
                    if (_price == null && _cp.base != null) _price = _cp.base;
                    if (_price == null) return null;
                    let _ded = 0;
                    if (document.getElementById('cust-notop')?.checked && _cp.noTop)    _ded += parseFloat(_cp.noTop)    || 0;
                    if (document.getElementById('cust-nobot')?.checked && _cp.noBottom) _ded += parseFloat(_cp.noBottom) || 0;
                    const _total = Math.round((parseFloat(_price) || 0) - _ded);
                    return { base: parseFloat(_price) || 0, extra: 0, divider: 0, customFee: 0, deduction: _ded, total: Math.max(FEES.minPrice, _total) };
                }
                // Format płaski (stary): jedna liczba dla wszystkich opcji
                const _flat = parseFloat(_cp);
                if (isNaN(_flat)) return null;
                return { base: _flat, extra: 0, divider: 0, customFee: 0, deduction: 0, total: Math.max(FEES.minPrice, _flat) };
            }
            if (shelfType === 'modular') {
                const moduleW = parseInt(moduleWidthSelect.value);
                const moduleH = parseInt(moduleHeightSelect.value);
...
function generateOrderCode() { const shelfTypeVal = shelfTypeSelect.value; const sideColorVal = sideColorSelect.value || ''; const shelfColorVal = shelfColorSelect.value || ''; if (!shelfTypeVal || !sideColorVal || !shelfColorVal) return null;
        ...
        function parseOrderCode(code) {
            ...
            if (parts[0] === 'c' && (parts.length === 6 || parts.length === 7)) {
                ...
                let price = 'brak ceny';
                {
                    const _cpD = (typeof customPrices !== 'undefined') ? customPrices[custType.id] : undefined;
                    if (_cpD !== undefined && _cpD !== null) {
                        if (typeof _cpD === 'object') {
                            const _pwD = _cpD.prices && _cpD.prices[String(width)];
                            let _pD = _pwD ? (_pwD[String(depth)] != null ? _pwD[String(depth)] : _pwD['10']) : null;
                            if (_pD == null && _cpD.base != null) _pD = _cpD.base;
                            if (_pD != null) price = _pD + ' zł';
                        } else {
                            price = _cpD + ' zł';
                        }
                    }
        ...
        async function showVisualDetailsForCode(code) {
            ...
            try {
                ...
                // Czekaj na zakończenie refreshDimensionArrows (setTimeout 120ms) i stabilizację sceny
                await new Promise(resolve => setTimeout(resolve, 400));

                const [snapshotDataUrl, priceDetails] = await Promise.all([
                    generate3dSnapshotFromCurrentModel(true),
    ...
    if (galleryData.length > 0) {
        ...
        function initializeReviews() { const reviews = [ '"Fantastyczna półeczka, przyszła od razu zmontowana i idealnie pasuje w wielu miejscach. Serdecznie polecam!"', '"Piękna, zgrabna półka. Solidnie wykonana, bez żadnych wad. Polecam!"', '"Bardzo ładna i funkcjonalna półka, świetnie zapakowana na czas transportu. Polecam!"', '"Super produkt – pięknie wykonany, idealnie pasuje do małych słoiczków z przyprawami w kuchni. Polecam!"', '"Ładna i solidna półka, starannie wykonana. Kolor świetnie wpasował się w meble. Polecam!"' ]; let currentReviewIndex = 0; const reviewDisplay = document.getElementById('customerReview'); const reviewDisplayDesktop = document.getElementById('customerReviewDesktop'); function showNextReview() { if (!reviewDisplay && !reviewDisplayDesktop) return; if (reviewDisplay) reviewDisplay.classList.remove('review-fade-enter-active'); if (reviewDisplayDesktop) reviewDisplayDesktop.classList.remove('review-fade-enter-active'); setTimeout(() => { const reviewData = reviews[currentReviewIndex]; const svgHtml = `<svg xmlns="http://www.w3.org/2000/svg" class="h-10 w-10 text-lime-500 inline-block mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M5 3v4M3 5h4M6 17v4m-2-2h4m5-16l2.286 6.857L21 12l-5.714 2.143L13 21l-2.286-6.857L5 12l5.714-2.143L13 3z" /></svg> ${reviewData}`; if (reviewDisplay) { reviewDisplay.innerHTML = svgHtml; reviewDisplay.classList.add('review-fade-enter-active'); } if (reviewDisplayDesktop) { reviewDisplayDesktop.innerHTML = svgHtml; reviewDisplayDesktop.classList.add('review-fade-enter-active'); } currentReviewIndex = (currentReviewIndex + 1) % reviews.length; }, 700); } if (reviewDisplay || reviewDisplayDesktop) { showNextReview(); setInterval(showNextReview, 6000); } }
        function _shelf3dStartIdleTimer() {
            // Auto-reset kamery wyłączony
        }
        ...
        function open3dPanel() { if (shelfContainer) {
            ...
            // Idle reset — zapisz pozycję startową po 800ms i uruchom timer
            setTimeout(() => {
                if (!camera || !controls || !shelfGroup) return;
                _idleResetCam = { px: camera.position.x, py: camera.position.y, pz: camera.position.z, tx: controls.target.x, ty: controls.target.y, tz: controls.target.z, rotY: shelfGroup.rotation.y, shelfY: shelfGroup.position.y };
                _shelf3dStartIdleTimer();
                const _cvs = renderer && renderer.domElement;
                if (_cvs && !_cvs._idleListenerAdded) {
                    _cvs._idleListenerAdded = true;
                    _cvs.addEventListener('touchstart',  _shelf3dStartIdleTimer, { passive: true });
                    _cvs.addEventListener('touchmove',   _shelf3dStartIdleTimer, { passive: true });
                    _cvs.addEventListener('pointerdown', _shelf3dStartIdleTimer, { passive: true });
                    _cvs.addEventListener('pointermove', _shelf3dStartIdleTimer, { passive: true });
                }
            }, 800);
    ...
    if (window.innerWidth < 768) {
        setTimeout(() => {
            if (shelfGroup && camera && controls) {
                if (window._saved3dCam) {
                    // Przywróć dokładną pozycję kamery i rotację sprzed zamknięcia
                    camera.position.copy(window._saved3dCam.pos);
                    controls.target.copy(window._saved3dCam.target);
                    shelfGroup.rotation.y = window._saved3dCam.rotY;
                    controls.update();
                } else {
                    // Pierwsze otwarcie — ustaw domyślny widok
                    const h = parseInt(heightSelect && heightSelect.value) || 60;
                    ...
                    // Zapamiętaj pozycję startową dla idle reset
                    _idleResetCam = {
                        px: camX, py: 0.5, pz: camZ,
                        tx: 0, ty: 0, tz: 0,
                        rotY: shelfGroup ? shelfGroup.rotation.y : 0
                    };
                }
                _lastInteraction3d = Date.now();
            }
        }, 100);
    }
        ...
        function close3dPanel() { if (shelfContainer) {
            ...
            // Zapisz pozycję kamery i rotację półki przed zamknięciem
            if (window.innerWidth < 768 && camera && controls && shelfGroup) {
                window._saved3dCam = {
 ...
 modularShelfIconsContainer = document.getElementById('modularShelfIconsContainer');
            ...
            try {
                ...
                if (sideColorSelect)  sideColorSelect.addEventListener('change',  () => { if(typeof shelf3dReapplyMaterials==='function') shelf3dReapplyMaterials(); if(typeof shelf3dApplyColorLightSmart==='function') shelf3dApplyColorLightSmart(); });
                if (shelfColorSelect) shelfColorSelect.addEventListener('change', () => { if(typeof shelf3dReapplyMaterials==='function') shelf3dReapplyMaterials(); if(typeof shelf3dApplyColorLightSmart==='function') shelf3dApplyColorLightSmart(); });
              ...
              if(typeof initializeTabbedGallery === 'function') initializeTabbedGallery(); else console.error("initializeTabbedGallery is not defined");
                ...
                // Podepnij przyciski modala Allegro
                if (cartSummaryModalOverlay) {
                    ...
                    if(copyAllCodesBtn) copyAllCodesBtn.addEventListener('click', () => {
                        ...
                        if(ta) { ta.select(); navigator.clipboard.writeText(ta.value); }
                    });
                    ...
                    const cartSummaryGoToAllegroBtn = document.getElementById('cartSummaryGoToAllegroBtn');
        ...
        document.getElementById('height')?.addEventListener('change', () => {
            ...
            setTimeout(() => { if (typeof window._shelf3dApplyLightForSize === 'function') window._shelf3dApplyLightForSize(); }, 1150);
        });
...
function updateDimensionOverlay() { /* handled by handleDimensionChange */ }
...
function enterDragMode() {
    ...
    // Wróć do widoku buildowania przed trybem edycji
    if (_idleResetCam && typeof gsap !== 'undefined') {
        if (_idleTimer) { clearTimeout(_idleTimer); _idleTimer = null; }
        gsap.to(camera.position, { x: _idleResetCam.px, y: _idleResetCam.py, z: _idleResetCam.pz, duration: 0.7, ease: 'power2.inOut', onUpdate: () => controls && controls.update() });
        gsap.to(controls.target,  { x: _idleResetCam.tx, y: _idleResetCam.ty, z: _idleResetCam.tz, duration: 0.7, ease: 'power2.inOut' });
        gsap.to(shelfGroup.rotation, { y: _idleResetCam.rotY, duration: 0.7, ease: 'power2.inOut' });
        gsap.to(shelfGroup.position, { y: _idleResetCam.shelfY, duration: 0.7, ease: 'power2.inOut' });
    }
    ...
    if (isMobile) {
        // MOBILE: overlay transparent — canvas stays in place, no resize, no camera jump
        ...
        // FAB elements — floating over existing 3D canvas
        if (overlay) {
            const _svgArrow = '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 20V10"/><path d="M8 14l4-4 4 4"/><path d="M4 4h16"/></svg>';
            const _svgReset = '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M3 12a9 9 0 1 0 3-6.7"/><path d="M3 4v5h5"/></svg>';
            const _svgDone  = '<svg viewBox="0 0 24 24" fill="none" stroke="white" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M5 13l4 4L19 7"/></svg>';
            const pill = document.createElement('div'); pill.className = 'drag-pill';
            pill.innerHTML = _svgArrow + 'tryb edycji półek';
            overlay.appendChild(pill);
            const fabReset = document.createElement('button'); fabReset.className = 'drag-fab-reset';
            fabReset.title = 'Przywróć domyślne'; fabReset.innerHTML = _svgReset;
            fabReset.onclick = () => resetShelfPositions();
            overlay.appendChild(fabReset);
            const fabDone = document.createElement('button'); fabDone.className = 'drag-fab-done';
            fabDone.innerHTML = _svgDone + 'Gotowe';
            fabDone.onclick = () => exitDragModeAndKeep();
            overlay.appendChild(fabDone);
            const hint = document.createElement('div'); hint.className = 'drag-hint';
            hint.innerHTML = _svgArrow + 'Przesuń półki palcem';
            overlay.appendChild(hint);
            setTimeout(() => { if (hint.parentNode) hint.parentNode.removeChild(hint); }, 3200);
        }
    } else {
        // DESKTOP: in-place overlay na wrapperze 3D
        ...
        if (wrapper) {
            ...
            dOverlay.style.cssText = 'position:absolute;top:0;left:0;right:0;bottom:0;z-index:20;pointer-events:none;border-radius:12px;border:1.5px solid rgba(22,163,74,0.4);';

            const pill = document.createElement('div'); pill.className = 'drag-pill';
            pill.style.pointerEvents = 'none';
            pill.innerHTML = _svgArrow + 'tryb edycji półek';
            dOverlay.appendChild(pill);

            const fabReset = document.createElement('button'); fabReset.className = 'drag-fab-reset';
            fabReset.style.pointerEvents = 'auto';
            fabReset.title = 'Przywróć domyślne'; fabReset.innerHTML = _svgReset;
            fabReset.onclick = () => resetShelfPositions();
            dOverlay.appendChild(fabReset);

            const fabDone = document.createElement('button'); fabDone.className = 'drag-fab-done';
            fabDone.style.pointerEvents = 'auto';
            fabDone.innerHTML = _svgDone + 'Gotowe';
            fabDone.onclick = () => exitDragModeAndKeep();
            dOverlay.appendChild(fabDone);

            wrapper.appendChild(dOverlay);

            const hint = document.createElement('div'); hint.className = 'drag-hint';
            hint.innerHTML = _svgArrow + 'Przesuń półki myszką';
            wrapper.appendChild(hint);
            setTimeout(() => { if (hint.parentNode) hint.parentNode.removeChild(hint); }, 3200);

            // Desktop: canvas nie zmienia rozmiaru — aktualizuj controls
            if (controls) controls.update();
        }
    }
...
function exitDragMode() {
    ...
    if (isMobile) {
        // MOBILE: canvas stays in place — just hide overlay and remove FABs
        const overlay = document.getElementById('dragModeOverlay');
        if (overlay) {
            overlay.classList.remove('active');
            overlay.querySelectorAll('.drag-pill, .drag-fab-reset, .drag-fab-done, .drag-hint').forEach(el => el.remove());
        }
    } else {
    ...
    // Kamera pozostaje w aktualnej pozycji — brak przeskoku po wyjściu z trybu edycji
}
...
function highlightShelves(on) {
    if (!shelfGroup) return;
    const HL = 0x16A34A;
    // Przywróć oryginalny emissive zapisany przy podświetleniu
    const _restoreEmis = (c) => {
        if (c.material && c.userData && c.userData._hlSaved) {
            if (c.material.emissive) c.material.emissive.setHex(c.userData._origEmis || 0x000000);
            c.material.emissiveIntensity = c.userData._origEmisInt || 0;
            c.material.needsUpdate = true;
            c.userData._hlSaved = false;
        }
    };
    // Usuń stare obrysy (legacy) + przywróć emissive
    shelfGroup.children.filter(c => c.name?.startsWith('_shelfEdge_')).forEach(c => {
        shelfGroup.remove(c); if (c.geometry) c.geometry.dispose(); if (c.material) c.material.dispose();
    });
    shelfGroup.children.forEach(c => {
        if (c.isMesh && c.name?.startsWith('internalShelf_')) {
            c.children.filter(ch => ch.name?.startsWith('_shelfEdge_')).forEach(ch => {
                c.remove(ch); if (ch.geometry) ch.geometry.dispose(); if (ch.material) ch.material.dispose();
            });
            _restoreEmis(c);
        }
    });
    ...
    shelfGroup.children.forEach(c => {
        if (c.isMesh && c.name?.startsWith('internalShelf_')) {
            let w, h, d;
            if (c.userData && c.userData.shelfMatInfo) {
                w = c.userData.shelfMatInfo.w; h = c.userData.shelfMatInfo.h; d = c.userData.shelfMatInfo.d;
            } else if (c.geometry && c.geometry.parameters) {
                w = c.geometry.parameters.width; h = c.geometry.parameters.height; d = c.geometry.parameters.depth;
            } else { return; }

            // 1) Delikatna poświata — lekkie zielone rozjaśnienie całej półki (emissive)
            if (c.material && c.material.emissive) {
                if (!c.userData._hlSaved) {
                    c.userData._origEmis = c.material.emissive.getHex();
                    c.userData._origEmisInt = c.material.emissiveIntensity || 0;
                    c.userData._hlSaved = true;
                }
                c.material.emissive.setHex(HL);
                c.material.emissiveIntensity = 0.2;
                c.material.needsUpdate = true;
            }

            // 2) Cienki zielony obrys przedniej ścianki
            const hw = w / 2, hh = h / 2, hd = d / 2, eps = 0.004;
            const p = [
                new THREE.Vector3(-hw, -hh, hd + eps),
                new THREE.Vector3( hw, -hh, hd + eps),
                new THREE.Vector3( hw,  hh, hd + eps),
                new THREE.Vector3(-hw,  hh, hd + eps),
                new THREE.Vector3(-hw, -hh, hd + eps)
            ...
            const mat = new THREE.LineBasicMaterial({ color: HL, transparent: true, opacity: 0.9, depthTest: false });
            const outline = new THREE.Line(geo, mat);
            outline.renderOrder = 999;
            outline.name = '_shelfEdge_' + c.name;
            c.add(outline);
        }
    });
}
...
function refreshDimensionArrows() {
    if (!shelfGroup) { _dimSegments = []; return; }
    // Reset segmentów overlayu SVG — zostaną odbudowane przez add*DimensionArrows
    _dimSegments = [];
    // Remove old arrows from main group (isLine/isSprite + mug shelf arrow meshes)
    const toRemove = shelfGroup.children.filter(c =>
        (c.isLine && !c.name?.startsWith('_shelfEdge_')) ||
        (c.isSprite && c.name && (c.name.startsWith('dimensionLabel_') || c.name.startsWith('dimensionCone_'))) ||
        (c.isMesh && c.name && (
            c.name.startsWith('mugHeightArrow_') ||
            c.name.startsWith('mugWidthArrow_') ||
            c.name.startsWith('dimensionCone_')
        ))
    );
    toRemove.forEach(c => {
...
// ─────────────────────────────────────────────────────────────────────────
//  PREMIUM DIMENSION OVERLAY (SVG nad canvasem)
//  Etykiety i strzałki wymiarowe rysujemy jako SVG NAD canvasem 3D — dzięki
//  temu nie obejmuje ich filtr CSS canvasa (saturate/hue-rotate/contrast),
//  więc zieleń jest żywa, a tekst ostry (jak w konfiguratorach premium).
//  Segmenty (_dimSegments) trzymają współrzędne LOKALNE względem rodzica 3D
//  i są rzutowane na ekran w każdej klatce (updateDimensionOverlay()).
// ─────────────────────────────────────────────────────────────────────────
let _dimSegments = [];
let _dimOverlaySvg = null;
const _DIM_SVGNS = 'http://www.w3.org/2000/svg';
let _dimMeasureCtx = null;

function _dimColorHex() {
    const v = (typeof _dimStrokeHex === 'function') ? _dimStrokeHex() : 0x16A34A;
    return '#' + v.toString(16).padStart(6, '0');
}

function _dimMeasure(label, fs) {
    if (!_dimMeasureCtx) _dimMeasureCtx = document.createElement('canvas').getContext('2d');
    _dimMeasureCtx.font = '400 ' + fs + 'px "Inter","Montserrat","Helvetica Neue",Arial,sans-serif';
    return _dimMeasureCtx.measureText(label).width;
}

function _ensureDimOverlay() {
    if (!threeJsCanvasWrapper) return null;
    if (_dimOverlaySvg && _dimOverlaySvg.parentNode === threeJsCanvasWrapper) return _dimOverlaySvg;
    const svg = document.createElementNS(_DIM_SVGNS, 'svg');
    svg.setAttribute('id', 'dimOverlaySvg');
    ...
    svg.innerHTML = '<defs><filter id="dimBadgeShadow" x="-60%" y="-60%" width="220%" height="220%">'
        + '<feDropShadow dx="0" dy="1.5" stdDeviation="2.4" flood-color="#0f172a" flood-opacity="0.30"/></filter></defs>'
        + '<g id="dimOverlayG"></g>';
    threeJsCanvasWrapper.appendChild(svg);
    _dimOverlaySvg = svg;
    return svg;
}

const _dimProjV = (typeof THREE !== 'undefined') ? new THREE.Vector3() : null;
function _dimProject(parent, x, y, z, W, H) {
    _dimProjV.set(x, y, z);
    parent.localToWorld(_dimProjV);
    _dimProjV.project(camera);
    return {
        x: (_dimProjV.x * 0.5 + 0.5) * W,
        y: (-_dimProjV.y * 0.5 + 0.5) * H,
        z: _dimProjV.z
    };
}

function updateDimensionOverlay() {
    if (!camera || !threeJsCanvasWrapper || !shelfGroup) return;
    // Jeśli brak wybranych wymiarów — wyczyść overlay i nie rysuj nic
    const _hv = (typeof heightSelect !== 'undefined' && heightSelect) ? heightSelect.value : '';
    const _wv = (typeof widthSelect !== 'undefined' && widthSelect) ? widthSelect.value : '';
    const _dv = (typeof depthSelect !== 'undefined' && depthSelect) ? depthSelect.value : '';
    if (!_hv || !_wv || !_dv) {
        const _svg0 = _ensureDimOverlay();
        const _g0 = _svg0 && _svg0.querySelector('#dimOverlayG');
        if (_g0 && _g0.innerHTML) _g0.innerHTML = '';
        return;
    }
    const W = threeJsCanvasWrapper.clientWidth;
    const H = threeJsCanvasWrapper.clientHeight;
    if (!W || !H) return;
    const svg = _ensureDimOverlay();
    if (!svg) return;
    const g = svg.querySelector('#dimOverlayG');
    if (!g) return;
    // Rysuj wyłącznie segmenty należące do AKTUALNEJ bryły (odrzuć stare/odłączone)
    const segs = (_dimSegments || []).filter(s => {
        let p = s.parent;
        while (p) { if (p === shelfGroup) return true; p = p.parent; }
        return false;
    });
    if (segs.length === 0) { if (g.innerHTML) g.innerHTML = ''; return; }
    const color = _dimColorHex();
    const isMob = window.innerWidth < 768;
    const maxFs = isMob ? 12 : 15;       // górny limit wielkości tekstu (px) — nie rośnie za bardzo na wysokich półkach
    const minFs = isMob ? 10 : 11;       // dolny limit — zawsze czytelny
    const _clamp = (v, a, b) => Math.max(a, Math.min(b, v));
    let out = '';
    const inlineSegs = segs.filter(s => !s.right);
    const rightSegs = segs.filter(s => s.right);
    for (const seg of inlineSegs) {
        if (!seg.parent) continue;
        const A = _dimProject(seg.parent, seg.ax, seg.ay, seg.z, W, H);
        const B = _dimProject(seg.parent, seg.bx, seg.by, seg.z, W, H);
        if (A.z > 1 || B.z > 1) continue;            // za kamerą
        let dx = B.x - A.x, dy = B.y - A.y;
        const len = Math.hypot(dx, dy) || 1;
        dx /= len; dy /= len;
        const px = -dy, py = dx;                       // wektor prostopadły
        // Rozmiar PROPORCJONALNY do odstępu. Styl: cienka przerywana linia,
        // smukłe groty, liczba w przerwie linii — bez ramki i bez białej obwódki.
        const fs = _clamp(len * 0.30, minFs, maxFs);
        const ah = _clamp(fs * 0.5, 5, 11);            // grot
        const aw = ah * 0.5;
        const lw = _clamp(fs * 0.09, 1, 1.7);          // cienka linia
        const dash = Math.max(3, fs * 0.30).toFixed(1);
        const gap = Math.max(2, fs * 0.22).toFixed(1);
        const minLen = ah * 2 + 4;
        const drawHeads = len >= minLen;
        const inset = drawHeads ? ah : 0;
        const l1x = A.x + dx * inset, l1y = A.y + dy * inset;
        const l2x = B.x - dx * inset, l2y = B.y - dy * inset;
        const M = _dimProject(seg.parent, (seg.ax + seg.bx) / 2, (seg.ay + seg.by) / 2, seg.z, W, H);
        const tw = _dimMeasure(seg.label, fs);
        const gapHalf = Math.abs(dx) * (tw / 2 + 4) + Math.abs(dy) * (fs * 0.62 + 3);
        const gAx = M.x - dx * gapHalf, gAy = M.y - dy * gapHalf;
        const gBx = M.x + dx * gapHalf, gBy = M.y + dy * gapHalf;
        const dashAttr = ' stroke="' + color + '" stroke-width="' + lw.toFixed(2)
            + '" stroke-dasharray="' + dash + ',' + gap + '" stroke-linecap="round"';
        if (len > gapHalf * 2 + 8) {
            out += '<line x1="' + l1x.toFixed(1) + '" y1="' + l1y.toFixed(1) + '" x2="' + gAx.toFixed(1) + '" y2="' + gAy.toFixed(1) + '"' + dashAttr + '/>';
            out += '<line x1="' + gBx.toFixed(1) + '" y1="' + gBy.toFixed(1) + '" x2="' + l2x.toFixed(1) + '" y2="' + l2y.toFixed(1) + '"' + dashAttr + '/>';
        } else {
            out += '<line x1="' + l1x.toFixed(1) + '" y1="' + l1y.toFixed(1) + '" x2="' + l2x.toFixed(1) + '" y2="' + l2y.toFixed(1) + '"' + dashAttr + '/>';
        }
        if (drawHeads) {
            out += '<path d="M ' + A.x.toFixed(1) + ' ' + A.y.toFixed(1)
                + ' L ' + (l1x + px * aw).toFixed(1) + ' ' + (l1y + py * aw).toFixed(1)
                + ' L ' + (l1x - px * aw).toFixed(1) + ' ' + (l1y - py * aw).toFixed(1)
                + ' Z" fill="' + color + '"/>';
            out += '<path d="M ' + B.x.toFixed(1) + ' ' + B.y.toFixed(1)
                + ' L ' + (l2x + px * aw).toFixed(1) + ' ' + (l2y + py * aw).toFixed(1)
                + ' L ' + (l2x - px * aw).toFixed(1) + ' ' + (l2y - py * aw).toFixed(1)
                + ' Z" fill="' + color + '"/>';
        }
        const halo = Math.max(2.5, fs * 0.28).toFixed(1);
        out += '<text x="' + M.x.toFixed(1) + '" y="' + M.y.toFixed(1) + '" text-anchor="middle" dominant-baseline="central" '
            + 'paint-order="stroke" stroke="#ffffff" stroke-width="' + halo + '" stroke-linejoin="round" '
            + 'font-family="Inter,Montserrat,Arial,sans-serif" font-weight="400" font-size="' + fs.toFixed(1)
            + '" fill="' + color + '">' + seg.label + '</text>';
    }

    // ── Wymiary wyniesione POZA obrys — oś i linie kotwiczone w 3D, więc biegną
    //    równo z pochyloną półką (łańcuch ciągły, bardzo delikatne linie) ──
    if (rightSegs.length) {
        const baseX = rightSegs[0].ax;          // prawa krawędź boku (= width/2)
        const axis3X = baseX + 0.7;             // oś ~7 cm poza obrysem
        const wCol = 'rgba(120,134,153,0.5)';   // linie pomocnicze — mega delikatne
        const bnd = [];                         // punkty osi na granicach półek (do ciągłej osi + kresek)
        const seenY = new Set();
        let ok = false;
        for (const seg of rightSegs) {
            if (!seg.parent) continue;
            for (const yy of [seg.ay, seg.by]) {
                const k = yy.toFixed(3);
                if (seenY.has(k)) continue;
                seenY.add(k);
                const Pb = _dimProject(seg.parent, baseX, yy, seg.z, W, H);
                const Pa = _dimProject(seg.parent, axis3X, yy, seg.z, W, H);
                if (Pb.z > 1 || Pa.z > 1) continue;
                ok = true;
                // linia pomocnicza biegnąca równo z półką (3D-pozioma → na ekranie pochylona)
                out += '<line x1="' + Pb.x.toFixed(1) + '" y1="' + Pb.y.toFixed(1) + '" x2="' + Pa.x.toFixed(1) + '" y2="' + Pa.y.toFixed(1) + '" stroke="' + wCol + '" stroke-width="0.6"/>';
                bnd.push(Pa);
            }
        }
        if (ok && bnd.length) {
            // ciągła oś — łączymy punkty graniczne w kolejności pionowej
            const axisPts = bnd.slice().sort((a, b) => a.y - b.y);
            for (let i = 0; i < axisPts.length - 1; i++) {
                out += '<line x1="' + axisPts[i].x.toFixed(1) + '" y1="' + axisPts[i].y.toFixed(1) + '" x2="' + axisPts[i + 1].x.toFixed(1) + '" y2="' + axisPts[i + 1].y.toFixed(1) + '" stroke="' + color + '" stroke-width="0.8"/>';
            }
            // ukośne kreski na granicach
            for (const P of bnd) {
                out += '<line x1="' + (P.x - 3.6).toFixed(1) + '" y1="' + (P.y + 3.6).toFixed(1) + '" x2="' + (P.x + 3.6).toFixed(1) + '" y2="' + (P.y - 3.6).toFixed(1) + '" stroke="' + color + '" stroke-width="0.9"/>';
            }
            // wartości — na środku wnęki, przy osi, pisane prosto (z „cm")
            for (const seg of rightSegs) {
                if (!seg.parent) continue;
                const Pm = _dimProject(seg.parent, axis3X, (seg.ay + seg.by) / 2, seg.z, W, H);
                const Pb1 = _dimProject(seg.parent, baseX, seg.ay, seg.z, W, H);
                const Pb2 = _dimProject(seg.parent, baseX, seg.by, seg.z, W, H);
                if (Pm.z > 1) continue;
                const gl = Math.hypot(Pb1.x - Pb2.x, Pb1.y - Pb2.y);
                const fs = _clamp(gl * 0.32, isMob ? 12 : 13, isMob ? 15 : 18);
                const halo = Math.max(2.5, fs * 0.3).toFixed(1);
                out += '<text x="' + (Pm.x + 9).toFixed(1) + '" y="' + Pm.y.toFixed(1) + '" text-anchor="start" dominant-baseline="central" '
                    + 'paint-order="stroke" stroke="#ffffff" stroke-width="' + halo + '" stroke-linejoin="round" '
                    + 'font-family="Inter,Montserrat,Arial,sans-serif" font-weight="400" font-size="' + fs.toFixed(1)
                    + '" fill="' + color + '">' + seg.label + '</text>';
            }
...
// Wymiary wypalone do bitmapy (Canvas2D) — używane przy snapshocie do koszyka /
// podsumowania / PDF, bo overlay SVG nie trafia do toDataURL canvasa 3D.
// Rysuje tym samym stylem co updateDimensionOverlay, ale dla dowolnej kamery.
function _drawDimsToCanvas(ctx, segments, cam, W, H) {
    if (!ctx || !segments || !segments.length || !cam) return;
    const _v = new THREE.Vector3();
    const proj = (parent, x, y, z) => {
        _v.set(x, y, z);
    ...
    const color = _dimColorHex();
    // Font skalowany tak jak live SVG overlay (maxFs=15px przy ~500px, proporcjonalnie dla HD)
    const maxFs = Math.round(H / 37);   // ~49px → 24px wyświetlone przy każdej liczbie półek
    const minFs = Math.round(H / 43);   // ~42px → wystarczający dla gęstych półek 80cm
    const clamp = (v, a, b) => Math.max(a, Math.min(b, v));
    const measure = (label, fs) => {
        ctx.font = '400 ' + fs + 'px "Inter","Montserrat","Helvetica Neue",Arial,sans-serif';
        return ctx.measureText(label).width;
    };
    const inlineSegs = segments.filter(s => !s.right);
    const rightSegs = segments.filter(s => s.right);
    for (const seg of inlineSegs) {
        if (!seg.parent) continue;
        const A = proj(seg.parent, seg.ax, seg.ay, seg.z);
        const B = proj(seg.parent, seg.bx, seg.by, seg.z);
        if (A.z > 1 || B.z > 1) continue;
        let dx = B.x - A.x, dy = B.y - A.y;
        const len = Math.hypot(dx, dy) || 1;
        dx /= len; dy /= len;
        const px = -dy, py = dx;
        const fs = clamp(len * 0.30, minFs, maxFs);
        const ah = clamp(fs * 0.5, 5, 11);
        const aw = ah * 0.5;
        const lw = clamp(fs * 0.09, 1, 1.7);
        const drawHeads = len >= ah * 2 + 4;
        const inset = drawHeads ? ah : 0;
        const l1x = A.x + dx * inset, l1y = A.y + dy * inset;
        const l2x = B.x - dx * inset, l2y = B.y - dy * inset;
        const M = proj(seg.parent, (seg.ax + seg.bx) / 2, (seg.ay + seg.by) / 2, seg.z);
        const tw = measure(seg.label, fs);
        const gapHalf = Math.abs(dx) * (tw / 2 + 4) + Math.abs(dy) * (fs * 0.62 + 3);
        // cienka, przerywana linia z przerwą na liczbę
        ctx.strokeStyle = color;
        ctx.lineWidth = lw;
        ctx.lineCap = 'round';
        ctx.setLineDash([Math.max(3, fs * 0.30), Math.max(2, fs * 0.22)]);
        if (len > gapHalf * 2 + 8) {
            ctx.beginPath(); ctx.moveTo(l1x, l1y); ctx.lineTo(M.x - dx * gapHalf, M.y - dy * gapHalf); ctx.stroke();
            ctx.beginPath(); ctx.moveTo(M.x + dx * gapHalf, M.y + dy * gapHalf); ctx.lineTo(l2x, l2y); ctx.stroke();
        } else {
        ...
        if (drawHeads) {
            ctx.fillStyle = color;
            ctx.beginPath(); ctx.moveTo(A.x, A.y); ctx.lineTo(l1x + px * aw, l1y + py * aw); ctx.lineTo(l1x - px * aw, l1y - py * aw); ctx.closePath(); ctx.fill();
            ctx.beginPath(); ctx.moveTo(B.x, B.y); ctx.lineTo(l2x + px * aw, l2y + py * aw); ctx.lineTo(l2x - px * aw, l2y - py * aw); ctx.closePath(); ctx.fill();
        }
        // zielona liczba (zwykła grubość) z białą obwódką dla czytelności
        ctx.font = '400 ' + fs + 'px "Inter","Montserrat","Helvetica Neue",Arial,sans-serif';
        ...
        ctx.strokeText(seg.label, M.x, M.y + 0.5);
        ctx.fillStyle = color;
        ctx.fillText(seg.label, M.x, M.y + 0.5);
    }

    // Wymiary wyniesione poza obrys — oś i linie kotwiczone w 3D (biegną równo z pochyloną półką)
    if (rightSegs.length) {
        const baseX = rightSegs[0].ax;
        const axis3X = baseX + 0.7;
        ctx.lineCap = 'butt';
        ctx.setLineDash([]);
        const bnd = [];
        const seenY = new Set();
        ...
        for (const seg of rightSegs) {
            ...
            for (const yy of [seg.ay, seg.by]) {
                ...
                if (seenY.has(k)) continue;
                seenY.add(k);
                const Pb = proj(seg.parent, baseX, yy, seg.z);
                const Pa = proj(seg.parent, axis3X, yy, seg.z);
                if (Pb.z > 1 || Pa.z > 1) continue;
                ok = true;
                ctx.strokeStyle = 'rgba(120,134,153,0.5)';
                ctx.lineWidth = 0.6;
        ...
        if (ok && bnd.length) {
            ...
            ctx.lineWidth = 0.8;
            ctx.beginPath();
            ctx.moveTo(axisPts[0].x, axisPts[0].y);
            for (let i = 1; i < axisPts.length; i++) ctx.lineTo(axisPts[i].x, axisPts[i].y);
            ctx.stroke();
            ctx.lineWidth = 0.9;
            for (const P of bnd) {
                ctx.beginPath(); ctx.moveTo(P.x - 3.6, P.y + 3.6); ctx.lineTo(P.x + 3.6, P.y - 3.6); ctx.stroke();
            }
            ctx.textAlign = 'left';
            ctx.textBaseline = 'middle';
            ctx.lineJoin = 'round';
            for (const seg of rightSegs) {
                if (!seg.parent) continue;
                const Pm = proj(seg.parent, axis3X, (seg.ay + seg.by) / 2, seg.z);
                const Pb1 = proj(seg.parent, baseX, seg.ay, seg.z);
                const Pb2 = proj(seg.parent, baseX, seg.by, seg.z);
                if (Pm.z > 1) continue;
                const gl = Math.hypot(Pb1.x - Pb2.x, Pb1.y - Pb2.y);
                const fs = clamp(gl * 0.32, 13, 18);
                ctx.font = '400 ' + fs + 'px "Inter","Montserrat","Helvetica Neue",Arial,sans-serif';
                ctx.lineWidth = Math.max(2.5, fs * 0.3);
                ctx.strokeStyle = '#ffffff';
                ctx.strokeText(seg.label, Pm.x + 9, Pm.y);
                ctx.fillStyle = color;
                ctx.fillText(seg.label, Pm.x + 9, Pm.y);
            }
        }
    }
}
...
async function startCompositeExport() {
    ...
    try {
        ...
        for (let i = 0; i < codes.length; i++) {
            ...
            // Żywa scena — identyczny wygląd jak na stronie głównej
            const snap = await generate3dSnapshotFromCurrentModel(false);
        ...
        window._rglNoAnim = false; // przywróć animację także przy błędzie
...
// ── SZABLON 1: karta marki (render lewo, dane prawo z oryginalnymi próbnikami, cena, pasek na dole) ──
async function generateSzablon1(details, snapDataUrl, priceDetails) {
    console.log('[Szablon1] nowa KWADRATOWA karta v2 (1240×1240, pasek3.png)');
    const W = 1240, H = 1240, PAD = 30;
    try { await Promise.all([document.fonts.load('700 34px Poppins'),document.fonts.load('500 20px Poppins'),document.fonts.load('400 14px Poppins')]); if (document.fonts.ready) await document.fonts.ready; } catch(e){}
    const loadImg = (src) => new Promise(res => { if(!src){return res(null);} const i=new Image(); i.onload=()=>res(i); i.onerror=()=>res(null); i.src=src; });
    const icoNames = ['paczka','wkretarka','listek','iskry','linijka','ciezarowka','tarcza','kropla'];
    const ico = {}; await Promise.all(icoNames.map(async n => { ico[n]=await loadImg('ikony/cut/'+n+'.png'); }));
    const snap = await loadImg(snapDataUrl);
    const pasek = await loadImg('img/pasek3.png');
    function texFor(name,hex){ try{ if(typeof COLORS==='undefined')return null; const e=COLORS.find(c=>(c.name===name)||((c.value||'').toUpperCase()===(hex||'').toUpperCase())); return e?e.textureImg:null; }catch(_e){return null;} }
    const sideTex = await loadImg(texFor(details.sideColor, details.sideColorValue));
    const shelfTex = await loadImg(texFor(details.shelfColor, details.shelfColorValue));

    const GRAY='#f4f5f6',WHITE='#ffffff',BORD='#e4e6e8',TXT='#2b3034',MUT='#9aa1a7',GREEN='#3a9a4a',WOOD='#cf9f63';
    const L=16+PAD, R=W-16-PAD, top=16+PAD;
    const pasW=R-L, pasH=pasek?Math.round(pasW*pasek.height/pasek.width):0;
    const pasY=H-PAD-pasH;
    const footY = pasY - 84;
    const ry0=top, rh=footY - top - 28;
    const RW=Math.round((R-L)*0.58);

    const cv=document.createElement('canvas'); cv.width=W; cv.height=H; const ctx=cv.getContext('2d');
    function rrect(x,y,x2,y2,r,fill,stroke,sw){ ctx.beginPath(); ctx.moveTo(x+r,y); ctx.arcTo(x2,y,x2,y2,r); ctx.arcTo(x2,y2,x,y2,r); ctx.arcTo(x,y2,x,y,r); ctx.arcTo(x,y,x2,y,r); ctx.closePath(); if(fill){ctx.fillStyle=fill;ctx.fill();} if(stroke){ctx.strokeStyle=stroke;ctx.lineWidth=sw||1;ctx.stroke();} }
    function T(x,y,s,wt,sz,col,al,ba){ ctx.font=wt+' '+sz+'px Poppins, Arial, sans-serif'; ctx.fillStyle=col; ctx.textAlign=al||'left'; ctx.textBaseline=ba||'top'; ctx.fillText(s,x,y); }
    function Tw(s,wt,sz){ ctx.font=wt+' '+sz+'px Poppins, Arial, sans-serif'; return ctx.measureText(s).width; }
    function dIco(n,x,y,sz){ if(ico[n]) ctx.drawImage(ico[n],x,y,sz,sz); }
    function swatch(tex,hex,cx,cy,r){ ctx.save(); ctx.beginPath(); ctx.arc(cx,cy,r,0,7); ctx.closePath(); ctx.clip(); if(tex){ const sc=Math.max(2*r/tex.width,2*r/tex.height); ctx.drawImage(tex,cx-r,cy-r,tex.width*sc,tex.height*sc);} else { ctx.fillStyle=hex||WOOD; ctx.fillRect(cx-r,cy-r,2*r,2*r);} ctx.restore(); ctx.lineWidth=2; ctx.strokeStyle=BORD; ctx.beginPath(); ctx.arc(cx,cy,r,0,7); ctx.stroke(); }

    ctx.fillStyle=WHITE; ctx.fillRect(0,0,W,H);
    rrect(16,16,W-16,H-16,22,GRAY,BORD,2);
    if(snap){
        // przytnij przezroczyste marginesy zrzutu, żeby półka wypełniała kadr (bez ramki, na tle karty)
        let bx=0,by=0,bw=snap.width,bh=snap.height;
        try {
            const oc=document.createElement('canvas'); oc.width=snap.width; oc.height=snap.height; const octx=oc.getContext('2d'); octx.drawImage(snap,0,0);
            const dd=octx.getImageData(0,0,snap.width,snap.height).data; let x0=snap.width,y0=snap.height,x1=0,y1=0,found=false;
            for(let y=0;y<snap.height;y+=2){ for(let x=0;x<snap.width;x+=2){ if(dd[(y*snap.width+x)*4+3]>16){ found=true; if(x<x0)x0=x; if(x>x1)x1=x; if(y<y0)y0=y; if(y>y1)y1=y; } } }
            if(found){ bx=x0; by=y0; bw=Math.max(1,x1-x0); bh=Math.max(1,y1-y0); }
        } catch(_e) {}
        const aW=RW, aH=rh, sc=Math.min(aW/bw, aH/bh), dw=bw*sc, dh=bh*sc;
        ctx.drawImage(snap, bx,by,bw,bh, L+(RW-dw)/2, ry0+(rh-dh)/2, dw, dh);
    }

    let dx=L+RW+38, dy=top+12;
    const m=(details.shelfTypeValue||details.shelfType||'').toLowerCase();
    const title=(m.includes('wisz')||m.includes('hang'))?'Półka wisząca':(m.includes('stoj')||m.includes('stand'))?'Półka stojąca':(m.includes('mug')||m.includes('kub'))?'Półka na kubki':(m.includes('modu'))?'Półka modułowa':(details.shelfType||'Półka');
    let tsz=46; while(tsz>30 && Tw(title,'700',tsz) > (R-dx)) tsz-=2;
    T(dx,dy,title,'700',tsz,TXT); dy+=tsz+18;
    T(dx,dy,'PROSTA · FUNKCJONALNA · UNIWERSALNA','500',14,MUT); dy+=46;
    const dimS=(details.width||details.totalWidth||'')+' × '+(details.height||'')+' × '+(details.depth||'')+' cm';
    const pw=Tw(dimS,'500',26)+96; rrect(dx,dy,dx+pw,dy+58,13,WHITE,'#dfe2e5',2); dIco('linijka',dx+12,dy+8,42); T(dx+64,dy+29,dimS,'500',26,'#5c636a','left','middle'); dy+=84;
    swatch(sideTex,details.sideColorValue,dx+23,dy+23,23); T(dx+62,dy+4,'KOLOR BOKÓW','500',14,MUT); T(dx+62,dy+22,details.sideColor||'—','500',22,TXT); dy+=62;
    swatch(shelfTex,details.shelfColorValue,dx+23,dy+23,23); T(dx+62,dy+4,'KOLOR PÓŁEK','500',14,MUT); T(dx+62,dy+22,details.shelfColor||'—','500',22,TXT); dy+=68;
    dy+=8; ctx.strokeStyle=BORD; ctx.lineWidth=2; ctx.beginPath(); ctx.moveTo(dx,dy); ctx.lineTo(R,dy); ctx.stroke(); dy+=26;
    const isStand=m.includes('stoj')||m.includes('stand');
    [['paczka','Złożona do wysyłki'],[isStand?'tarcza':'wkretarka',isStand?'Gotowa do postawienia na blacie':'Montaż na 2 wkręty'],['tarcza','Trwała konstrukcja']].forEach(f=>{ dIco(f[0],dx,dy,52); T(dx+66,dy+26,f[1],'500',23,TXT,'left','middle'); dy+=70; });

    let fy=footY; ctx.strokeStyle=BORD; ctx.lineWidth=2; ctx.beginPath(); ctx.moveTo(L,fy); ctx.lineTo(R,fy); ctx.stroke(); fy+=18;
    T(L,fy,'KOD KONFIGURACYJNY','500',13,MUT);
    ctx.font='20px "Courier New",monospace'; ctx.fillStyle='#3a3f44'; ctx.textAlign='left'; ctx.textBaseline='top'; ctx.fillText(details.code||'—',L,fy+20);
    const totalRaw=(priceDetails&&priceDetails.total)?priceDetails.total:0;
    const disc=(typeof DISCOUNTS!=='undefined'&&DISCOUNTS.discount1item)?DISCOUNTS.discount1item:0.10;
    const rabat=totalRaw*disc, doZap=totalRaw-rabat; const fmt=n=>n.toFixed(2).replace('.',',')+' zł';
    T(R-320,fy,'RABATY','500',13,MUT); T(R-320,fy+20,'−'+fmt(rabat),'500',20,GREEN);
    ctx.strokeStyle=BORD; ctx.lineWidth=2; ctx.beginPath(); ctx.moveTo(R-190,fy-2); ctx.lineTo(R-190,fy+54); ctx.stroke();
    T(R,fy,'ŁĄCZNIE DO ZAPŁATY','500',13,MUT,'right'); T(R,fy+22,fmt(doZap),'700',34,GREEN,'right');

    if(pasek){ ctx.drawImage(pasek, L, pasY, pasW, pasH); }
    return new Promise(res => cv.toBlob(b=>res(b),'image/jpeg',0.95));
}

async function generateZestaw(shelves) {
    const N = shelves.length;
    console.log('[Zestaw] karta zestawu, polek: ' + N);
    try { await Promise.all([document.fonts.load('700 44px Poppins'),document.fonts.load('500 24px Poppins'),document.fonts.load('400 18px Poppins')]); if (document.fonts.ready) await document.fonts.ready; } catch(e){}
    const loadImg = (src) => new Promise(res => { if(!src){return res(null);} const i=new Image(); i.onload=()=>res(i); i.onerror=()=>res(null); i.src=src; });
    const icoNames = ['paczka','wkretarka','listek','iskry','linijka','ciezarowka','tarcza','kropla'];
    const ico = {}; await Promise.all(icoNames.map(async n => { ico[n]=await loadImg('ikony/cut/'+n+'.png'); }));
    const pasek4 = await loadImg('img/pasek4.png');
    function texFor(name,hex){ try{ if(typeof COLORS==='undefined')return null; const e=COLORS.find(c=>(c.name===name)||((c.value||'').toUpperCase()===(hex||'').toUpperCase())); return e?e.textureImg:null; }catch(_e){return null;} }
    const snaps = await Promise.all(shelves.map(s=>loadImg(s.snap)));
    const sideTex = await Promise.all(shelves.map(s=>loadImg(texFor(s.details.sideColor,s.details.sideColorValue))));
    const shelfTex = await Promise.all(shelves.map(s=>loadImg(texFor(s.details.shelfColor,s.details.shelfColorValue))));

    const GRAY='#f4f5f6',WHITE='#ffffff',BORD='#e4e6e8',TXT='#2b3034',MUT='#9aa1a7',GREEN='#3a9a4a',WOOD='#cf9f63';
    const PAD=30, cellW=(N===1?540:460), gap=44, kronoW=340;
    const W = 16+PAD + N*cellW + gap + kronoW + PAD+16;
    const L=16+PAD, R=W-16-PAD, top=16+PAD;
    const rowY=top+(N===1?168:112), renderH=(N===1?600:560), capH=200;
    const footY = rowY+renderH+capH+20;
    const footBottom = footY+70;
    const H = footBottom + PAD;
    ...
    ctx.fillStyle=WHITE; ctx.fillRect(0,0,W,H);
    rrect(16,16,W-16,H-16,22,WHITE,BORD,2);

    // Tytuł zestawu
    const m0=(shelves[0].details.shelfTypeValue||shelves[0].details.shelfType||'').toLowerCase();
    const allSame=shelves.every(s=>((s.details.shelfTypeValue||s.details.shelfType||'').toLowerCase())===m0);
    let titleStr, subStr;
    if(N===1){
        titleStr=(m0.includes('wisz')||m0.includes('hang'))?'Półka wisząca':(m0.includes('stoj')||m0.includes('stand'))?'Półka stojąca':(m0.includes('kub')||m0.includes('mug'))?'Półka na kubki':(m0.includes('modu'))?'Półka modułowa':(shelves[0].details.shelfType||'Półka');
        subStr='PROSTA · FUNKCJONALNA · UNIWERSALNA';
    } else {
        const tw=allSame?((m0.includes('wisz')||m0.includes('hang'))?' wiszących':(m0.includes('stoj')||m0.includes('stand'))?' stojących':(m0.includes('kub')||m0.includes('mug'))?' na kubki':''):'';
        titleStr='Zestaw '+N+' półek'+tw;
        subStr='PROSTE · FUNKCJONALNE · UNIWERSALNE';
    }
    T(L,top,titleStr,'700',44,TXT);
    T(L,top+60,subStr,'500',15,MUT);
    // Pojedyncza półka: wymiary jako pigułka pod nagłówkiem głównym (zamiast „PÓŁKA 1" pod renderem)
    if(N===1){
        const _d0=shelves[0].details;
        const _dimS0=(_d0.width||_d0.totalWidth||'')+' × '+(_d0.height||'')+' × '+(_d0.depth||'')+' cm';
        const _pw0=Tw(_dimS0,'500',24)+76, _py0=top+96;
        rrect(L,_py0,L+_pw0,_py0+48,11,WHITE,'#dfe2e5',2); dIco('linijka',L+10,_py0+7,34); T(L+54,_py0+24,_dimS0,'500',24,TXT,'left','middle');
    }
    // Wykryj montaż wiszący: po typie półki ORAZ po polu MONTAŻ (options / raw.mount) — np. „Półka na kubki" z montażem wiszącym
    const _isHang=d=>{ const t=(((d&&d.shelfTypeValue)||'')+' '+((d&&d.shelfType)||'')+' '+((d&&d.options)||'')).toLowerCase(); if(t.includes('wisz')||t.includes('hang'))return true; const r=(d&&d.raw)||{}; return r.mount==='hanging'||r.mountType==='h'||(d&&d.mountType)==='h'; };
    function wrapLines(s,wt,sz,maxW){ const ws=s.split(' '); const ls=[]; let cur=''; ws.forEach(w=>{ const t=cur?cur+' '+w:w; if(Tw(t,wt,sz)<=maxW){cur=t;}else{ if(cur)ls.push(cur); cur=w; } }); if(cur)ls.push(cur); return ls; }

    // Wspólny kadr dla wszystkich renderów — żeby stały równo (ta sama skala i ta sama wysokość)
    function bboxOf(snap){ if(!snap) return null; let bx=0,by=0,bw=snap.width,bh=snap.height; try{ const oc=document.createElement('canvas'); oc.width=snap.width; oc.height=snap.height; const octx=oc.getContext('2d'); octx.drawImage(snap,0,0); const dd=octx.getImageData(0,0,snap.width,snap.height).data; let x0=snap.width,y0=snap.height,x1=0,y1=0,fnd=false; for(let y=0;y<snap.height;y+=2){ for(let x=0;x<snap.width;x+=2){ if(dd[(y*snap.width+x)*4+3]>16){ fnd=true; if(x<x0)x0=x; if(x>x1)x1=x; if(y<y0)y0=y; if(y>y1)y1=y; } } } if(fnd){ bx=x0; by=y0; bw=Math.max(1,x1-x0); bh=Math.max(1,y1-y0); } }catch(_e){} return {bx,by,bw,bh}; }
    let uX=1e9,uY=1e9,uX2=0,uY2=0,anyB=false;
    snaps.forEach(sn=>{ const b=bboxOf(sn); if(b){ anyB=true; uX=Math.min(uX,b.bx); uY=Math.min(uY,b.by); uX2=Math.max(uX2,b.bx+b.bw); uY2=Math.max(uY2,b.by+b.bh); } });
    const _sref=snaps.find(x=>x); if(!anyB){ uX=0; uY=0; uX2=_sref?_sref.width:1; uY2=_sref?_sref.height:1; }
    const ubx=uX,uby=uY,ubw=Math.max(1,uX2-uX),ubh=Math.max(1,uY2-uY);
    const _scU=Math.min((cellW-16)/ubw, renderH/ubh), _dwU=ubw*_scU, _dhU=ubh*_scU, _ryU=rowY+(renderH-_dhU)/2;

    // Render row + podpisy pod każdą półką
    for(let i=0;i<N;i++){
        const s=shelves[i], snap=snaps[i];
        const cellL=L+i*cellW, cx=cellL+cellW/2;
        if(snap){ ctx.drawImage(snap, ubx,uby,ubw,ubh, cx-_dwU/2, _ryU, _dwU, _dhU); }
        let cy=rowY+renderH+10;
        if(N>1){
            T(cx,cy,'PÓŁKA '+(i+1),'700',14,MUT,'center'); cy+=26;
            const dimS=(s.details.width||s.details.totalWidth||'')+' × '+(s.details.height||'')+' × '+(s.details.depth||'')+' cm';
            const pw=Tw(dimS,'500',24)+76; rrect(cx-pw/2,cy,cx+pw/2,cy+46,11,WHITE,'#dfe2e5',2); dIco('linijka',cx-pw/2+10,cy+6,34); T(cx-pw/2+50,cy+23,dimS,'500',24,TXT,'left','middle'); cy+=62;
        }
        const same=(s.details.sideColor===s.details.shelfColor);
        if(same){
            const nm=s.details.sideColor||'—', nw=Tw(nm,'500',22), gx=cx-(nw+44)/2;
            swatch(sideTex[i],s.details.sideColorValue,gx+16,cy+16,16); T(gx+44,cy+16,nm,'500',22,TXT,'left','middle');
        } else {
            const n1='Boki: '+(s.details.sideColor||'—'), w1=Tw(n1,'500',17), gx1=cx-(w1+38)/2;
            swatch(sideTex[i],s.details.sideColorValue,gx1+14,cy+14,14); T(gx1+36,cy+14,n1,'500',17,TXT,'left','middle'); cy+=32;
            const n2='Półki: '+(s.details.shelfColor||'—'), w2=Tw(n2,'500',17), gx2=cx-(w2+38)/2;
            swatch(shelfTex[i],s.details.shelfColorValue,gx2+14,cy+14,14); T(gx2+36,cy+14,n2,'500',17,TXT,'left','middle');
        }
        // Notatka montażowa pod kolorami — tylko dla półek wiszących
        cy += 42;
        if(_isHang(s.details)){
            const mtxt = (N===1) ? 'Kołki do powieszenia w komplecie' : 'Kołki do powieszenia w zestawie';
            const mls = wrapLines(mtxt,'500',16,cellW-110);
            let mw=0; mls.forEach(l=>{ mw=Math.max(mw,Tw(l,'500',16)); });
            const bx=cx-(40+mw)/2;
            dIco('wkretarka',bx,cy,32);
            mls.forEach((l,li)=> T(bx+40, cy+(mls.length>1?1:8)+li*20, l,'500',16,TXT,'left','top'));
        }
    }

    // Prawa kolumna — wspólne cechy
    const colBottom=rowY+renderH+capH-30;
    // Pionowa kolumna PŁYTA — gotowy pasek4.png (kolumna „W ZESTAWIE" usunięta)
    const kx=L+N*cellW+gap;
    ctx.strokeStyle=BORD; ctx.lineWidth=2; ctx.beginPath(); ctx.moveTo(kx-gap/2,top); ctx.lineTo(kx-gap/2,colBottom); ctx.stroke();
    if(pasek4){ const kAvailH=colBottom-top; const sc=Math.min(kronoW/pasek4.width, kAvailH/pasek4.height)*0.792; const nw=pasek4.width*sc, nh=pasek4.height*sc; ctx.drawImage(pasek4, kx+(kronoW-nw)+10, top+(kAvailH-nh)/2, nw, nh); }

    // Stopka — kody + suma cen
    let fy2=footY; ctx.strokeStyle=BORD; ctx.lineWidth=2; ctx.beginPath(); ctx.moveTo(L,fy2); ctx.lineTo(R,fy2); ctx.stroke(); fy2+=16;
    T(L,fy2,N===1?'KOD KONFIGURACYJNY':'KODY KONFIGURACYJNE','500',13,MUT);
    const codes=shelves.map(s=>s.details.code||'—').join('   +   ');
    let cf=18; ctx.font=cf+'px "Courier New",monospace'; while(cf>9 && ctx.measureText(codes).width>(R-600-L)){ cf--; ctx.font=cf+'px "Courier New",monospace'; }
    ctx.fillStyle='#3a3f44'; ctx.textAlign='left'; ctx.textBaseline='top'; ctx.fillText(codes,L,fy2+20);

    const prices=shelves.map(s=>(s.price&&s.price.total)?s.price.total:0);
    const subtotal=prices.reduce((a,b)=>a+b,0);
    const D=(typeof DISCOUNTS!=='undefined')?DISCOUNTS:{discount1item:0.10,discountBest:0.10,discountCheap:0.25};
    let rabat=0; if(N===1){ rabat=subtotal*D.discount1item; } else { rabat=Math.max.apply(null,prices)*D.discountBest + Math.min.apply(null,prices)*D.discountCheap; }
    const doZap=subtotal-rabat; const fmt=n=>n.toFixed(2).replace('.',',')+' zł';
    T(R-580,fy2,'WARTOŚĆ','500',13,MUT); T(R-580,fy2+20,fmt(subtotal),'500',20,'#7b828a');
    T(R-340,fy2,'RABATY','500',13,MUT); T(R-340,fy2+20,'−'+fmt(rabat),'500',20,GREEN);
    ctx.strokeStyle=BORD; ctx.lineWidth=2; ctx.beginPath(); ctx.moveTo(R-200,fy2-2); ctx.lineTo(R-200,fy2+54); ctx.stroke();
    T(R,fy2,'ŁĄCZNIE DO ZAPŁATY','500',13,MUT,'right'); T(R,fy2+22,fmt(doZap),'700',34,GREEN,'right');
...
async function startCompositeExport() {
    ...
    try {
        ...
        for (let i = 0; i < codes.length; i++) {
            ...
            window._rglNoAnim = true; // wyłącz animację składania na czas zrzutu — model od razu kompletny
            await reconfigureFromDetails(details);
            // Czekaj aż animacja składania faktycznie się skończy (różne typy mają różny czas;
            // przy półce na kubki 900 ms było za krótkie → łapana rozłożona).
            {
                const _t0 = Date.now();
                await new Promise(r => setTimeout(r, 400)); // daj animacji wystartować
                while (currentAnimationTimeline && typeof currentAnimationTimeline.isActive === 'function'
                       && currentAnimationTimeline.isActive() && (Date.now() - _t0) < 7000) {
                    await new Promise(r => setTimeout(r, 60));
                }
                await new Promise(r => setTimeout(r, 250)); // krótka stabilizacja sceny
            }

            // Dokończ animację PRZEGRÓDEK — mają osobne tweeny gsap poza głównym timeline,
            // więc czekanie ich nie łapie. Wymuś finalną pozę (pozycja/skala/krycie).
            if (shelfGroup) {
                shelfGroup.traverse(o => {
                    if (o.isMesh && o.name && o.name.indexOf('divider_') === 0) {
                        try { if (window.gsap) { gsap.killTweensOf(o.position); gsap.killTweensOf(o.scale); if (o.material) gsap.killTweensOf(o.material); } } catch(_e){}
                        if (o.userData && typeof o.userData._pfxFinalY === 'number') o.position.y = o.userData._pfxFinalY;
                        o.scale.set(o.scale.x, 1, o.scale.z);
        ...
        window._rglNoAnim = false; // przywróć normalną animację w żywym podglądzie
        // Każda liczba półek (też 1 szt.) → ten sam układ „zestaw"
        let blob = await generateZestaw(shelves);

        statusDiv.innerHTML = '';
        const successRow = document.createElement('div');
        successRow.style.cssText = 'display:flex;align-items:center;gap:10px;padding:14px 16px;border-radius:12px;background:#f0fdf4;border:1.5px solid #bbf7d0;flex-wrap:wrap;';
        successRow.innerHTML = `
            <svg style="width:26px;height:26px;flex-shrink:0;color:#16a34a;" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
            <div style="flex:1"><div style="font-size:14px;font-weight:700;color:#15803d;">Gotowe!</div><div style="font-size:12px;color:#16a34a;margin-top:2px;">${codes.length} półk${codes.length===1?'a':codes.length<5?'i':'i'} · łącznie z rabatami</div></div>
            <button id="compositeDownloadBtn" style="padding:9px 16px;background:#16a34a;color:#fff;border:none;border-radius:9px;font-size:12px;font-weight:700;cursor:pointer;">${codes.length===1?'↓ Pobierz kartę JPG':'↓ Pobierz kartę zestawu JPG'}</button>
        `;
        document.getElementById('batchItemsContainer').appendChild(successRow);
        document.getElementById('compositeDownloadBtn').onclick = () => {
            const url = URL.createObjectURL(blob);
            const a = document.createElement('a');
            a.href = url; a.download = (codes.length===1 ? ('polka-karta-'+((shelves[0].details.code||'allegro').substring(0,18))) : ('polki-zestaw-'+codes.length+'szt')) + '.jpg'; a.click();
            setTimeout(() => URL.revokeObjectURL(url), 2000);
        };
...
// Przechwytuje żywy renderer (te same materiały/oświetlenie co podgląd na stronie)
async function captureLiveRenderer(targetW, targetH) {
    return new Promise(resolve => {
        // Zapamiętaj aktualny rozmiar i pozycję kamery
        ...
        // Wymuś identyczną pozę dla KAŻDEGO zrzutu — wspólny lekki kąt 3/4 (a nie 0 „na wprost"),
        // żeby półki w zestawie były równo, a górne narożniki czytały się jako czysty róg (nie szew).
        const _snapRotY = -0.22; // wspólny kąt obrotu modelu (rad); do dostrojenia (np. -0.12 … -0.32)
        const _capRot = shelfGroup ? { x: shelfGroup.rotation.x, y: shelfGroup.rotation.y, z: shelfGroup.rotation.z } : null;
        if (shelfGroup) { shelfGroup.rotation.set(0, _snapRotY, 0); shelfGroup.updateMatrixWorld(true); }

        // reset3DView() ustawia kamerę dokładnie tak jak przy normalnym podglądzie
        if (typeof reset3DView === 'function') reset3DView();

        // Użyj docelowego aspect ratio karty — obraz pasuje bez zniekształceń
        const cardAspect = (targetW && targetH) ? (targetW / targetH) : ((oldW && oldH) ? (oldW / oldH) : 1);
        const HD_H = 1800;
        const HD_W = Math.round(HD_H * cardAspect);

        // Tymczasowo ustaw HD z aspect ratio karty
        renderer.setPixelRatio(Math.min(window.devicePixelRatio, 2));
        renderer.setSize(HD_W, HD_H);
        camera.aspect = cardAspect;
        camera.updateProjectionMatrix();

        // Zmniejsz etykiety wymiarów tylko na czas eksportu
        const _DIM_SCALE = 0.45;
        const _dimSprites = [];
        scene.traverse(obj => {
            if (obj.isSprite && obj.name && obj.name.startsWith('dimensionLabel_')) {
                _dimSprites.push({ obj, sx: obj.scale.x, sy: obj.scale.y, sz: obj.scale.z });
                obj.scale.set(obj.scale.x * _DIM_SCALE, obj.scale.y * _DIM_SCALE, obj.scale.z * _DIM_SCALE);
            }
        });
        ...
        const _oldBgHd = scene.background; scene.background = null;
        renderer.render(scene, camera);
        ...
        controls.update();
        if (shelfGroup && _capRot) { shelfGroup.rotation.set(_capRot.x, _capRot.y, _capRot.z); shelfGroup.updateMatrixWorld(true); }
        renderer.render(scene, camera);
...
async function generateCompositeImage(shelves) {
    const N = shelves.length;
    // Output 1400×1200px dla Allegro — szersze karty, półki dobrze wyglądają
    const OUT_W = 1400;
    const OUT_H = 1200;
    const PAD = 12;
    const PRICE_BAR_H = 110;
    ...
    shelves.forEach((shelf, idx) => {
        ...
        // Render 3D — bez zoomu, źródło 1800px > cel ~1144px → zawsze ostry
        ...
        const codeBoxH = (d.customPositions && d.customPositions.length > 0) ? 60 : 46;
        ctx.fillStyle = '#f9fafb';
        ctx.beginPath(); ctx.roundRect(x + 14, codeBoxY, SHELF_W - 28, codeBoxH, 6); ctx.fill();
        ctx.strokeStyle = '#f3f4f6'; ctx.lineWidth = 1;
        ctx.beginPath(); ctx.roundRect(x + 14, codeBoxY, SHELF_W - 28, codeBoxH, 6); ctx.stroke();
        ...
        if (d.customPositions && d.customPositions.length > 0) {
            const thickness = 18;
            const pts = [...d.customPositions].sort((a,b)=>a-b).map(v => (thickness + v*10) + 'mm');
            const placementStr = pts.join(' · ');
            ctx.fillStyle = '#111827'; ctx.font = '400 11px Arial,sans-serif';
            ctx.fillText(placementStr, x + 22, codeBoxY + 50);
        }
    });
...
// Helper: zwraca kolor tekstu etykiety wymiaru (biały na ciemnych płytach, czarny na jasnych)
function _dimLabelColor() {
    const hex = (sideColorSelect && sideColorSelect.value) ? sideColorSelect.value : '#ffffff';
    const r = parseInt(hex.slice(1,3), 16) || 0;
    const g = parseInt(hex.slice(3,5), 16) || 0;
    const b = parseInt(hex.slice(5,7), 16) || 0;
    return (0.299*r + 0.587*g + 0.114*b) < 80 ? '#ffffff' : '#111111';
}

// Kolor linii wymiarowych — szary (pasuje do każdego koloru płyty)
function _dimStrokeHex() {
    return 0x16A34A; // premium green — stały kolor niezależny od koloru półki
}

// Premium badge sprite — biały półprzezroczysty badge (≈78%) + zielony tekst Inter SemiBold
// Styl: konfiguratory mebli premium (IKEA Planner / Tylko / Blum) + rysunek techniczny CAD
function _makeDimChipSprite(label, mobileScale) {
    const SCALE = 8;                       // supersampling dla ostrego tekstu
    const fpx = 50 * SCALE;                // ~20px tekstu po przeskalowaniu sprite'a
    const FONT = '700 ' + fpx + 'px "Inter","Montserrat","Helvetica Neue",Arial,sans-serif';
    const STROKE = _dimStrokeHex ? '#' + _dimStrokeHex().toString(16).padStart(6, '0') : '#16A34A';
    const _tmp = document.createElement('canvas').getContext('2d');
    _tmp.font = FONT;
    const tw = _tmp.measureText(label).width;
    const padX = 17 * SCALE;
    const padY = 10 * SCALE;
    const radius = 9 * SCALE;
    const margin = 9 * SCALE;              // zapas na miękki cień (box-shadow)
    const W = Math.ceil(tw + padX * 2 + margin * 2);
    const H = Math.ceil(fpx + padY * 2 + margin * 2);
    const bw = W - margin * 2;             // szerokość badge
    const bh = H - margin * 2;             // wysokość badge
    const canvas = document.createElement('canvas');
    canvas.width = W; canvas.height = H;
    const ctx = canvas.getContext('2d');
    ctx.clearRect(0, 0, W, H);
    // Biały półprzezroczysty badge z delikatnym cieniem (box-shadow)
    ctx.save();
    ctx.shadowColor = 'rgba(15,23,42,0.22)';
    ctx.shadowBlur = 8 * SCALE;
    ctx.shadowOffsetX = 0;
    ctx.shadowOffsetY = 2.5 * SCALE;
    const x0 = margin, y0 = margin, x1 = margin + bw, y1 = margin + bh;
    ctx.beginPath();
    ctx.moveTo(x0 + radius, y0); ctx.lineTo(x1 - radius, y0);
    ...
    ctx.closePath();
    ctx.fillStyle = 'rgba(255,255,255,0.95)';   // prawie pełna biel — czytelne na drewnie
    ctx.fill();
    ctx.restore();
    // Wyraźny kolorowy obrys badge dla technicznego, czytelnego wyglądu
    ctx.save();
    ctx.lineWidth = 2 * SCALE;
    ctx.strokeStyle = 'rgba(22,163,74,0.55)';
    ctx.stroke();
    ctx.restore();
    // Zielony tekst — kolor identyczny jak linia wymiarowa
    ctx.font = FONT;
    ctx.textAlign = 'center';
    ctx.textBaseline = 'middle';
    ctx.fillStyle = STROKE;
    ctx.fillText(label, W / 2, H / 2 + 1 * SCALE);
    ...
    const ms = mobileScale || (window.innerWidth < 768 ? 2.0 : 1.3);
    const baseH = 0.225 * ms * (H / (fpx + padY * 2)); // zachowaj rozmiar tekstu mimo marginesu cienia
    sprite.scale.set(baseH * (W / H), baseH, 1);
...
function addMugShelfCompartmentHeightArrow(group, widthNum, width, depth, thickness, levelData) {
    ...
    // Strzałka wewnątrz półki — tak samo jak addShelfDimensionArrows
    let xPos;
    if (widthNum == 84) {
        const innerW = width - 2 * thickness;
        xPos = (-innerW / 2 + 3 * (1.25 + thickness)) + 0.06;
    } else {
        xPos = width / 2 - thickness - 0.12;
    }
    const y1 = levelData.bottom;
    const y2 = levelData.top;
    // Overlay SVG (poza filtrem canvasa) — żywa zieleń, ostry tekst
    _dimSegments.push({ parent: group, ax: xPos, ay: y1, bx: xPos, by: y2, z: zPos, label: label });
}
...
function addMugShelfCompartmentWidthArrow(group, widthNum, width, depth, thickness, bottomPanel, lowestInternalShelf) {
    ...
    // Overlay SVG — pozioma strzałka szerokości komory
    _dimSegments.push({ parent: group, ax: x1, ay: yPos, bx: x2, by: yPos, z: zPos, label: label });
}
...
function addModularDimensionArrows(mainGroup) {
    if (!mainGroup) return;
    const thickness = 0.18;
    const lineMat = new THREE.LineDashedMaterial({ color: _dimStrokeHex(), linewidth: 1, dashSize: 0.06, gapSize: 0.04 });

    function makeLabel(text, x, y, z, scaleX) {
        const sprite = _makeDimChipSprite(text);
        sprite.position.set(x, y, z);
        sprite.name = 'dimensionLabel_modular_' + text.replace(' ','_');
        mainGroup.add(sprite);
    }

    const _addDashedLine = (pts) => {
    ...
    function addVerticalArrow(y1, y2, xPos, zPos, labelX) {
        ...
        // Overlay SVG — oś wymiarowa centralnie w module (linia + etykieta wyrównane)
        _dimSegments.push({ parent: mainGroup, ax: labelX, ay: y1, bx: labelX, by: y2, z: zPos, label: label });
    }
...
function addShelfDimensionArrows(group, internalShelves, bottomPanel, topPanel, thickness, width, height, depth) {
    // Usuń wcześniejsze segmenty tej samej bryły — zapobiega nakładaniu się
    // starych wymiarów przy ponownej przebudowie (zmiana wysokości itp.)
    if (Array.isArray(_dimSegments)) _dimSegments = _dimSegments.filter(s => s.parent !== group);
    const sorted = [...internalShelves].sort((a, b) => a.position.y - b.position.y);

    // Kotwica przy PRAWEJ krawędzi półki — wymiary wynoszone poza obrys, do prawej kolumny
    const xPos = (width / 2 - thickness) * 0.62;   // tylko do detekcji półek przesuniętych
    const rightX = width / 2;                       // zewnętrzna prawa krawędź boku
    const zPos = depth ? depth / 2 + 0.02 : 0.02;

    // Wykryj wzór niestandardowy
    const _xTol = 0.01;
    ...
    // Rejestrujemy segmenty wymiarowe do overlayu SVG (rysowane nad canvasem,
    // poza filtrem CSS — żywa zieleń + ostry tekst). Bez obiektów 3D.
    levels.forEach((lvl) => {
        const gapCm = Math.round((lvl.y2 - lvl.y1) * 100) / 10;
        if (gapCm <= 0) return;
        const gapLabel = (Number.isInteger(gapCm) ? gapCm : gapCm.toFixed(1)) + ' cm';
        const segH = lvl.y2 - lvl.y1;
        const _isMob = window.innerWidth < 768;
        const _minGap = _isMob ? 0.06 : 0.04;
        if (segH < _minGap) return;
        _dimSegments.push({ parent: group, ax: xPos, ay: lvl.y1, bx: xPos, by: lvl.y2, z: zPos, label: gapLabel });
    });
}
...
(function() {
    ...
    function onTouchMove(e) {
        if (!_vt0 || !shelfGroup || dragModeActive) return;
        if (e.touches.length !== 1) return;
        ...
        if (_isVertical === null) {
            if (Math.abs(dx) > LOCK_THRESHOLD || Math.abs(dy) > LOCK_THRESHOLD) {
                _isVertical = Math.abs(dy) > Math.abs(dx);
                if (_isVertical) controls.enabled = false;
            }
            return;
        }
        if (!_isVertical) return;
    ...
    function onTouchEnd(e) {
        if (_isVertical) controls.enabled = true;
        _vt0 = null; _vy0 = null; _isVertical = null;
    }

    function attachMobilePan() {
        const canvas = renderer && renderer.domElement;
        if (!canvas) { setTimeout(attachMobilePan, 500); return; }
        canvas.addEventListener('touchstart', onTouchStart, { passive: true });
        canvas.addEventListener('touchmove', onTouchMove, { passive: false });
        canvas.addEventListener('touchend', onTouchEnd, { passive: true });
        canvas.addEventListener('touchcancel', onTouchEnd, { passive: true });
    }
    window.addEventListener('load', () => setTimeout(attachMobilePan, 800));
})();

// ---- RAPORT PDF DO DRUKU ----
function getDrillPoints(details) {
    const heightMM  = details.height * 10;
    const thickness = 18;
    const noTop = details.raw && details.raw.noTopShelf;
    const noBot = details.raw && details.raw.noBottomShelf;
    const points = [];

    if (!noBot) points.push({ y: thickness / 2, label: 'Podstawa' });

    if (details.customPositions && details.customPositions.length > 0) {
        [...details.customPositions].sort((a,b)=>a-b).forEach((pos,i) => {
            points.push({ y: thickness + pos * 10, label: `Półka ${i+1}` });
        });
    } else {
        const shelfCount = details.shelfCount || 0;
        const topT = noTop ? 0 : thickness;
        const botT = noBot ? 0 : thickness;
        const availH = heightMM - topT - botT;
        const spaceBetween = shelfCount > 0 ? (availH - shelfCount * thickness) / (shelfCount + 1) : 0;
        let currentY = noBot ? 0 : thickness;
        for (let i = 1; i <= shelfCount; i++) {
            currentY += spaceBetween;
            points.push({ y: currentY + thickness / 2, label: `Półka ${i}` });
            currentY += thickness;
        }
    }

    if (!noTop) points.push({ y: heightMM - thickness / 2, label: 'Góra' });
    return points.sort((a,b) => b.y - a.y);
}

function buildDrillingSVG(details) {
    const heightMM  = details.height * 10;
    const depthMM   = details.depth  * 10;
    const thickness = 18;
    const edgeDist  = depthMM < 70 ? depthMM/2 : depthMM < 120 ? 25 : depthMM <= 250 ? 37 : 50;
    const middleDist = depthMM - 2 * edgeDist;
    const isSingleHole = depthMM < 70;

    // Oblicz pozycje osi wiercenia (mm od dołu)
    const drillPoints = [];
    drillPoints.push({ y: thickness / 2, label: 'Podstawa' });

    if (details.customPositions && details.customPositions.length > 0) {
        // Własne rozmieszczenie z kodu cp...
        [...details.customPositions].sort((a,b)=>a-b).forEach((pos,i) => {
            drillPoints.push({ y: thickness + pos * 10, label: `Półka ${i+1}` });
        });
    } else {
        // Równomierne rozmieszczenie — identyczna logika jak kalkulator
        const shelfCount = details.shelfCount || 0;
        const noTop = details.raw && details.raw.noTopShelf;
        const noBot = details.raw && details.raw.noBottomShelf;
        const topT  = noTop ? 0 : thickness;
        ...
        for (let i = 1; i <= shelfCount; i++) {
            currentY += spaceBetween;
            drillPoints.push({ y: currentY + thickness / 2, label: `Półka ${i}` });
            currentY += thickness;
        }
    }
    drillPoints.push({ y: heightMM - thickness / 2, label: 'Góra' });

    // SVG wymiary
    const BW = 130, BH = 420;
    const TM = 38, LM = 14, RS = 160, BotM = 14;
    const SW = LM + BW + RS, SH = TM + BH + BotM;
    const sc = BH / heightMM;
    const bL = LM, bR = LM+BW, bT = TM, bBot = TM+BH;
    const xFr = bL + (isSingleHole ? BW/2 : (edgeDist/depthMM)*BW);
    const xBk = bL + (isSingleHole ? BW/2 : BW - (edgeDist/depthMM)*BW);
    const dimY = bT - 8, tkH = 6;

    let dimBar = '';
    if (isSingleHole) {
        dimBar = `<line x1="${bL}" y1="${dimY}" x2="${bR}" y2="${dimY}" stroke="#dc2626" stroke-width="1"/>
        <text x="${(bL+bR)/2}" y="${dimY-6}" text-anchor="middle" font-size="9" fill="#dc2626" font-weight="700" font-family="Arial,sans-serif">Otwór na środku</text>`;
    } else {
        dimBar = `<line x1="${bL}" y1="${dimY}" x2="${bR}" y2="${dimY}" stroke="#dc2626" stroke-width="1"/>
        <line x1="${bL}" y1="${dimY-tkH/2}" x2="${bL}" y2="${dimY+tkH/2}" stroke="#dc2626" stroke-width="1"/>
        <line x1="${xFr}" y1="${dimY-tkH/2}" x2="${xFr}" y2="${dimY+tkH/2}" stroke="#dc2626" stroke-width="1"/>
        <line x1="${xBk}" y1="${dimY-tkH/2}" x2="${xBk}" y2="${dimY+tkH/2}" stroke="#dc2626" stroke-width="1"/>
        <line x1="${bR}" y1="${dimY-tkH/2}" x2="${bR}" y2="${dimY+tkH/2}" stroke="#dc2626" stroke-width="1"/>
        <text x="${(bL+xFr)/2}" y="${dimY-6}" text-anchor="middle" font-size="9" fill="#dc2626" font-weight="700" font-family="Arial,sans-serif">${edgeDist}mm</text>
        <text x="${(xFr+xBk)/2}" y="${dimY-6}" text-anchor="middle" font-size="9" fill="#9ca3af" font-family="Arial,sans-serif">(${middleDist.toFixed(0)}mm)</text>
        <text x="${(xBk+bR)/2}" y="${dimY-6}" text-anchor="middle" font-size="9" fill="#dc2626" font-weight="700" font-family="Arial,sans-serif">${edgeDist}mm</text>`;
    }

    let axisLines = `<line x1="${xFr}" y1="${bT}" x2="${xFr}" y2="${bBot}" stroke="#dc2626" stroke-width="0.8" stroke-dasharray="3,3" opacity="0.5"/>`;
    if (!isSingleHole) axisLines += `<line x1="${xBk}" y1="${bT}" x2="${xBk}" y2="${bBot}" stroke="#dc2626" stroke-width="0.8" stroke-dasharray="3,3" opacity="0.5"/>`;

    let drillSVG = '';
    drillPoints.forEach(dp => {
        const sy = bT + BH - dp.y * sc;
        const bH2 = Math.max(1.5, thickness * sc);
        drillSVG += `<rect x="${bL}" y="${(sy-bH2/2).toFixed(1)}" width="${BW}" height="${bH2.toFixed(1)}" fill="#374151" opacity="0.16"/>`;
        drillSVG += `<line x1="${bL}" y1="${sy.toFixed(1)}" x2="${bR}" y2="${sy.toFixed(1)}" stroke="#dc2626" stroke-width="1" stroke-dasharray="4,3"/>`;
        drillSVG += `<circle cx="${xFr}" cy="${sy.toFixed(1)}" r="4" fill="#dc2626" stroke="#7f1d1d" stroke-width="0.8"/>`;
        if (!isSingleHole) drillSVG += `<circle cx="${xBk}" cy="${sy.toFixed(1)}" r="4" fill="#dc2626" stroke="#7f1d1d" stroke-width="0.8"/>`;
        drillSVG += `<line x1="${bR}" y1="${sy.toFixed(1)}" x2="${bR+18}" y2="${sy.toFixed(1)}" stroke="#dc2626" stroke-width="0.8"/>`;
        drillSVG += `<text x="${bR+22}" y="${(sy+3.5).toFixed(1)}" font-size="10" font-weight="700" fill="#b91c1c" font-family="Arial,sans-serif">${dp.y.toFixed(1)} mm</text>`;
        drillSVG += `<text x="${bR+88}" y="${(sy+3.5).toFixed(1)}" font-size="9" fill="#6b7280" font-family="Arial,sans-serif">(${dp.label})</text>`;
    });

    return `<svg width="${SW}" height="${SH}" viewBox="0 0 ${SW} ${SH}" xmlns="http://www.w3.org/2000/svg">
  <rect x="${bL}" y="${bT}" width="${BW}" height="${BH}" fill="#fefce8" stroke="#1f2937" stroke-width="1.5"/>
  ${dimBar}${axisLines}${drillSVG}
</svg>`;
}

function openPrintReport(shelves) {
    const n = shelves.length;
    const date = new Date().toLocaleDateString('pl-PL');
    const perPage = n === 1 ? 1 : 2;

    // Oblicz rabaty identycznie jak calculateCartTotal():
    // 1 półka: -10% na tę półkę
    // 2+ półek: -10% na najdroższą, -25% na najtańszą
    const allPrices = shelves.map((s, i) => ({ idx: i, base: s.price && s.price.total ? s.price.total : 0 }));
    const discounts = new Array(n).fill(0);
    if (n === 1) {
        discounts[0] = Math.round(allPrices[0].base * 0.10);
    } else if (n >= 2) {
        const sorted = [...allPrices].sort((a, b) => b.base - a.base);
        discounts[sorted[0].idx] = Math.round(sorted[0].base * 0.10);
        discounts[sorted[sorted.length - 1].idx] = Math.round(sorted[sorted.length - 1].base * 0.25);
    }

    const cards = shelves.map((shelf, idx) => {
        const d = shelf.details;
        const p = shelf.price;
        const basePrice = p && p.total ? p.total : 0;
        const discount = discounts[idx];
        const finalPrice = basePrice - discount;

        // Rozmieszczenie — standard czy własne
        const isCustom = d.customPositions && d.customPositions.length > 0;
        const isCharged = p && p.customFee && p.customFee > 0;
        let placementBadge = '';
        if (isCustom) {
            placementBadge = `<span class="badge-custom">Własne rozmieszczenie${isCharged ? ' +50 zł' : ''}</span>`;
        } else {
            placementBadge = `<span class="badge-std">Standardowe</span>`;
        }

        const noTop = d.raw && d.raw.noTopShelf;
        const noBot = d.raw && d.raw.noBottomShelf;

        let placementInfo = '';
        if (isCustom) {
            placementInfo = [...d.customPositions].sort((a,b)=>a-b).map(v=>v+'cm').join(' · ');
        } else {
            const thickness = 18, heightMM = d.height * 10, sc = d.shelfCount || 0;
            const topT = noTop ? 0 : thickness, botT = noBot ? 0 : thickness;
            if (sc > 0) {
                const sp = ((heightMM - topT - botT) - sc * thickness) / (sc + 1);
                placementInfo = 'Co ' + sp.toFixed(0) + ' mm';
            }
        }

        let optionsTags = '';
        if (noTop) optionsTags += `<span class="tag-warn">Bez górnej półki</span>`;
        if (noBot) optionsTags += `<span class="tag-warn">Bez dolnej półki</span>`;

        const drillPoints = getDrillPoints(d);
        const depthMM = d.depth * 10;
        const edgeDist = depthMM < 70 ? depthMM/2 : depthMM < 120 ? 25 : depthMM <= 250 ? 37 : 50;

        const drillRows = drillPoints.map(pt =>
            `<div class="dr"><span class="dl">${pt.label}</span><span class="dv">${pt.y.toFixed(1)} mm</span></div>`
        ).join('');

        const priceHtml = discount > 0
            ? `<span class="price-old">${basePrice} zł</span> <span class="price-final">${finalPrice} zł</span>`
            : `<span class="price-final">${finalPrice} zł</span>`;

        const breakAfter = (idx + 1) % 2 === 0 && idx < n - 1;

        return `<div class="card${breakAfter ? ' pg-break' : ''}">
  <div class="card-head">
    <span class="cn">#${idx+1}${n > 1 ? ' / '+n : ''}</span>
    <span class="cc">${d.code || ''}</span>
    <div class="price-wrap">${priceHtml}</div>
  </div>
  <div class="card-body">
    <div class="col-img">
      <img src="${shelf.snap}" alt="3D"/>
    </div>
    <div class="col-info">
      <div class="section-lbl">Parametry</div>
      <table class="pt">
        <tr><td class="pk">Wymiary</td><td class="pv">${d.height*10} × ${d.width ? d.width*10 : '?'} × ${d.depth*10} mm</td></tr>
        <tr><td class="pk">Liczba półek</td><td class="pv">${d.shelfCount || 0} szt.</td></tr>
        <tr><td class="pk">Kolor boku</td><td class="pv">${d.sideColor || '—'}</td></tr>
        <tr><td class="pk">Kolor półek</td><td class="pv">${d.shelfColor || '—'}</td></tr>
        <tr><td class="pk">Rozmieszczenie</td><td class="pv">${placementInfo || '—'} &nbsp;${placementBadge}</td></tr>
        ${optionsTags ? `<tr><td class="pk">Opcje</td><td class="pv warn">${optionsTags}</td></tr>` : ''}
      </table>
      <div class="divider"></div>
      <div class="section-lbl">Wiercenie — od dołu</div>
      <div class="drill-grid">${drillRows}</div>
      <div class="drill-foot">Odl. od krawędzi: ${edgeDist} mm</div>
    </div>
  </div>
</div>`;
    });

    const cardH = perPage === 1 ? '272mm' : '132mm';

    const html = `<!DOCTYPE html><html lang="pl"><head><meta charset="UTF-8">
<title>Zestawienie półek — ${date}</title>
<style>
*{box-sizing:border-box;margin:0;padding:0}
body{font-family:Arial,sans-serif;background:#fff;color:#111}
.card{width:100%;height:${cardH};display:flex;flex-direction:column;border-bottom:1px solid #e5e7eb}
.pg-break{page-break-after:always}
.card-head{display:flex;align-items:center;gap:10px;padding:4px 8px;background:#f9fafb;border-bottom:1px solid #e5e7eb;flex-shrink:0}
.cn{font-size:10px;font-weight:700;color:#6b7280;white-space:nowrap}
.cc{font-family:monospace;font-size:12px;font-weight:700;color:#1f2937;flex:1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
.price-wrap{display:flex;align-items:baseline;gap:6px;white-space:nowrap}
.price-old{font-size:11px;color:#9ca3af;text-decoration:line-through}
.price-final{font-size:16px;font-weight:700;color:#16a34a}
.card-body{display:grid;grid-template-columns:55% 1fr;flex:1;min-height:0}
.col-img{border-right:1px solid #e5e7eb;overflow:hidden;display:flex;align-items:center;justify-content:center;background:#f8f8f8}
.col-img img{width:100%;height:100%;object-fit:contain}
.col-info{padding:8px 10px;overflow:hidden;display:flex;flex-direction:column;gap:6px}
.section-lbl{font-size:8px;font-weight:700;color:#1f2937;text-transform:uppercase;letter-spacing:.07em}
.pt{border-collapse:collapse;width:100%}
.pk{font-size:11px;color:#1f2937;padding:1px 0;white-space:nowrap;vertical-align:top}
.pv{font-size:11px;font-weight:600;color:#1f2937;padding:1px 0 1px 8px}
.warn{color:#92400e;font-size:10px}
.badge-custom{background:#fef3c7;color:#92400e;font-size:9px;font-weight:700;padding:1px 6px;border-radius:8px;white-space:nowrap}
.badge-std{background:#f0fdf4;color:#166534;font-size:9px;font-weight:700;padding:1px 6px;border-radius:8px;white-space:nowrap}
.divider{border-top:0.5px solid #e5e7eb;margin:2px 0}
.drill-grid{display:grid;grid-template-columns:1fr 1fr;gap:1px 8px}
.dr{display:flex;justify-content:space-between;align-items:baseline;border-bottom:0.5px solid #f3f4f6;padding:1px 0}
.dl{font-size:11px;color:#1f2937}
.dv{font-family:monospace;font-size:12px;font-weight:700;color:#dc2626}
.drill-foot{font-size:8px;color:#1f2937;margin-top:2px}
.tag-warn{background:#fef3c7;color:#92400e;font-size:9px;font-weight:700;padding:1px 6px;border-radius:8px;display:inline-block;margin-right:4px}
...
// ============================================================
// SCHEMAT WIERCENIA — otwiera nowe okno do druku / zapisu PDF
// Wygląd wzorowany na Kalkulator_Schematu_Wiercenia.html
// ============================================================
function openDrillingSchema(details) {
    const heightMM   = details.height * 10;
    const depthMM    = details.depth  * 10;
    const thickness  = 18;
    const edgeDist   = depthMM < 70 ? depthMM/2 : depthMM < 120 ? 25 : depthMM <= 250 ? 37 : 50;
    ...
    // Pozycje osi wiercenia mm od dołu boku — identyczna logika jak kalkulator
    const drillPoints = [];
    drillPoints.push({ y: thickness / 2, label: 'Podstawa' });
    [...details.customPositions].sort((a,b)=>a-b).forEach((pos,i) => {
        drillPoints.push({ y: thickness + pos * 10, label: `Półka ${i+1}` });
    ...
    // SVG dimensions
    const BOARD_PX_W  = 200;
    const BOARD_PX_H  = 650;
    const TOP_MARGIN  = 48;
    const LEFT_MARGIN = 20;
    const RIGHT_SPACE = 240;
    const BOT_MARGIN  = 20;
    const SVG_W = LEFT_MARGIN + BOARD_PX_W + RIGHT_SPACE;
    const SVG_H = TOP_MARGIN + BOARD_PX_H + BOT_MARGIN;
    const scale = BOARD_PX_H / heightMM;
    const boardLeft  = LEFT_MARGIN;
    const boardRight = LEFT_MARGIN + BOARD_PX_W;
    const boardTop   = TOP_MARGIN;
    const boardBot   = TOP_MARGIN + BOARD_PX_H;

    const xFrontRel = isSingleHole ? BOARD_PX_W/2 : (edgeDist/depthMM)*BOARD_PX_W;
    const xBackRel  = isSingleHole ? BOARD_PX_W/2 : BOARD_PX_W - xFrontRel;
    const xFront = boardLeft + xFrontRel;
    const xBack  = boardLeft + xBackRel;

    // Pasek wymiarowy
    const dimBarY = boardTop - 10;
    const tickH = 8;
    let dimBarSVG = '';
    if (isSingleHole) {
        dimBarSVG = `
            <line x1="${boardLeft}" y1="${dimBarY}" x2="${boardRight}" y2="${dimBarY}" stroke="#dc2626" stroke-width="1"/>
            <line x1="${boardLeft}" y1="${dimBarY-tickH/2}" x2="${boardLeft}" y2="${dimBarY+tickH/2}" stroke="#dc2626" stroke-width="1"/>
            <line x1="${boardRight}" y1="${dimBarY-tickH/2}" x2="${boardRight}" y2="${dimBarY+tickH/2}" stroke="#dc2626" stroke-width="1"/>
            <text x="${(boardLeft+boardRight)/2}" y="${dimBarY-9}" text-anchor="middle" font-size="11" fill="#dc2626" font-weight="700" font-family="Arial,sans-serif">Otwór na środku</text>`;
    } else {
        dimBarSVG = `
            <line x1="${boardLeft}" y1="${dimBarY}" x2="${boardRight}" y2="${dimBarY}" stroke="#dc2626" stroke-width="1"/>
            <line x1="${boardLeft}" y1="${dimBarY-tickH/2}" x2="${boardLeft}" y2="${dimBarY+tickH/2}" stroke="#dc2626" stroke-width="1"/>
            <line x1="${xFront}" y1="${dimBarY-tickH/2}" x2="${xFront}" y2="${dimBarY+tickH/2}" stroke="#dc2626" stroke-width="1"/>
            <line x1="${xBack}" y1="${dimBarY-tickH/2}" x2="${xBack}" y2="${dimBarY+tickH/2}" stroke="#dc2626" stroke-width="1"/>
            <line x1="${boardRight}" y1="${dimBarY-tickH/2}" x2="${boardRight}" y2="${dimBarY+tickH/2}" stroke="#dc2626" stroke-width="1"/>
            <text x="${(boardLeft+xFront)/2}" y="${dimBarY-9}" text-anchor="middle" font-size="11" fill="#dc2626" font-weight="700" font-family="Arial,sans-serif">${edgeDist} mm</text>
            <text x="${(xFront+xBack)/2}" y="${dimBarY-9}" text-anchor="middle" font-size="11" fill="#9ca3af" font-family="Arial,sans-serif">(${middleDist.toFixed(1)} mm)</text>
            <text x="${(xBack+boardRight)/2}" y="${dimBarY-9}" text-anchor="middle" font-size="11" fill="#dc2626" font-weight="700" font-family="Arial,sans-serif">${edgeDist} mm</text>`;
    }

    // Pionowe linie osi głębokości
    let axisSVG = `<line x1="${xFront}" y1="${boardTop}" x2="${xFront}" y2="${boardBot}" stroke="#dc2626" stroke-width="1" stroke-dasharray="4,4" opacity="0.5"/>`;
    if (!isSingleHole) axisSVG += `<line x1="${xBack}" y1="${boardTop}" x2="${xBack}" y2="${boardBot}" stroke="#dc2626" stroke-width="1" stroke-dasharray="4,4" opacity="0.5"/>`;

    // Linie wiercenia + bandy + kropki + etykiety
    let drillSVG = '';
    drillPoints.forEach(dp => {
        const svgY  = boardTop + BOARD_PX_H - dp.y * scale;
        const bandH = Math.max(2, thickness * scale);
        const bandY = svgY - bandH / 2;
        drillSVG += `<rect x="${boardLeft}" y="${bandY.toFixed(1)}" width="${BOARD_PX_W}" height="${bandH.toFixed(1)}" fill="#374151" opacity="0.18"/>`;
        drillSVG += `<line x1="${boardLeft}" y1="${svgY.toFixed(1)}" x2="${boardRight}" y2="${svgY.toFixed(1)}" stroke="#dc2626" stroke-width="1.2" stroke-dasharray="5,3"/>`;
        drillSVG += `<circle cx="${xFront}" cy="${svgY.toFixed(1)}" r="5" fill="#dc2626" stroke="#7f1d1d" stroke-width="1"/>`;
        if (!isSingleHole) drillSVG += `<circle cx="${xBack}" cy="${svgY.toFixed(1)}" r="5" fill="#dc2626" stroke="#7f1d1d" stroke-width="1"/>`;
        drillSVG += `<line x1="${boardRight}" y1="${svgY.toFixed(1)}" x2="${boardRight+28}" y2="${svgY.toFixed(1)}" stroke="#dc2626" stroke-width="1"/>`;
        drillSVG += `<text x="${boardRight+34}" y="${(svgY+4).toFixed(1)}" font-size="13" font-weight="700" fill="#b91c1c" font-family="Arial,sans-serif">${dp.y.toFixed(1)} mm</text>`;
        drillSVG += `<text x="${boardRight+120}" y="${(svgY+4).toFixed(1)}" font-size="12" fill="#6b7280" font-family="Arial,sans-serif">(${dp.label})</text>`;
    });

    const sumHTML = `<div style="display:flex;gap:24px;flex-wrap:wrap;font-size:13px;border-bottom:2px solid #e5e7eb;padding-bottom:12px;margin-bottom:16px;">
        <div><span style="font-size:11px;color:#9ca3af;text-transform:uppercase;letter-spacing:.04em;display:block">Wymiar boku</span>${heightMM} × ${depthMM} mm</div>
        <div><span style="font-size:11px;color:#9ca3af;text-transform:uppercase;letter-spacing:.04em;display:block">Grubość płyty</span>${thickness} mm</div>
        <div><span style="font-size:11px;color:#9ca3af;text-transform:uppercase;letter-spacing:.04em;display:block">Odl. od krawędzi</span>${edgeDist} mm</div>
        <div><span style="font-size:11px;color:#9ca3af;text-transform:uppercase;letter-spacing:.04em;display:block">Dyst. między otworami</span>${isSingleHole ? 'otwór na środku' : middleDist.toFixed(1)+' mm'}</div>
        <div><span style="font-size:11px;color:#9ca3af;text-transform:uppercase;letter-spacing:.04em;display:block">Półki wewnętrzne</span>${details.customPositions.length} szt.</div>
    </div>`;

    const html = `<!DOCTYPE html><html lang="pl"><head><meta charset="UTF-8">
<title>Schemat Wiercenia — ${details.code}</title>
<style>*{box-sizing:border-box;margin:0;padding:0}body{font-family:Arial,sans-serif;background:#fff;color:#111;padding:24px 32px}h1{font-size:20px;font-weight:800;margin-bottom:3px;color:#1f2937}.sub{font-size:12px;color:#9ca3af;margin-bottom:14px;font-family:monospace}.note{margin-top:14px;font-size:11px;color:#9ca3af}@media print{body{padding:8mm 10mm}@page{margin:5mm;size:A4 portrait}*{-webkit-print-color-adjust:exact!important;print-color-adjust:exact!important}}</style>
<body>
<h1>Schemat Wiercenia</h1>
<div class="sub">${details.code || ''}</div>
${sumHTML}
<svg width="${SVG_W}" height="${SVG_H}" viewBox="0 0 ${SVG_W} ${SVG_H}" xmlns="http://www.w3.org/2000/svg">
  <rect x="${boardLeft}" y="${boardTop}" width="${BOARD_PX_W}" height="${BOARD_PX_H}" fill="#fefce8" stroke="#1f2937" stroke-width="2"/>
  ${dimBarSVG}${axisSVG}${drillSVG}
</svg>
<p class="note">Schemat poglądowy — wymiary w mm od dolnej krawędzi boku półki. Drukuj w orientacji pionowej.</p>
</body></html>`;
    const w = window.open('', '_blank');
    if (w) { w.document.write(html); w.document.close(); } else { alert('Zezwól na otwieranie okien popup, aby wyświetlić schemat.'); }
}
