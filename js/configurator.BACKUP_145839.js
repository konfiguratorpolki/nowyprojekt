...
function fitCameraToShelf() {
   ...
   function rebuildAndAnimateIn(config, withRotationAnimation = true) {
      ...
      const _animDur = withRotationAnimation ? 1 : 0;
      ...
      return new Promise(resolve => {
            ...
            if (shelfType === 'modular') {
                ...
                masterTimeline.add(() => {
                    ...
                    let _mugScale = (4.84 / _mugMaxDim);
                    _mugScale = Math.min(Math.max(_mugScale, 0.5), 1.3);
                ...
                masterTimeline.add(() => {
                    ...
                    const _mobOff = 0;
                    shelfGroup.position.y = _mobOff;
                    const _patScale = Array.isArray(config.shelves) ? 1.1 * 0.8 : 1.1;
        ...
        async function updatePreview(animateChanges = true) {
            ...
            if (!leftSide || !rightSide) {
                ...
                if (shelfType === 'mug_shelf') {
                    ...
                    let _newMugScale = 4.84 / _newMugMaxDim;
                    _newMugScale = Math.min(Math.max(_newMugScale, 0.5), 1.3);
...
function generateOrderCode() { const shelfTypeVal = shelfTypeSelect.value; const sideColorVal = sideColorSelect.value || ''; const shelfColorVal = shelfColorSelect.value || ''; if (!shelfTypeVal || !sideColorVal || !shelfColorVal) return null;
        ...
        // ── KARTA MARKA (nowy układ: render+dane, pasek płyty, stopka) — generuje JPG ──
        async function generateBrandCard(details, snapDataUrl, priceDetails) {
            const W = 1240, H = 762, PAD = 30;
            try {
                await Promise.all([
            ...
            const ico = {};
            await Promise.all(icoNames.map(async n => { ico[n] = await loadImg('ikony/cut/' + n + '.png'); }));
            const snap = await loadImg(snapDataUrl);
            const plyta = await loadImg('img/plyta.png');

            const GRAY_BG='#f4f5f6', WHITE='#ffffff', BORD='#e4e6e8', TXT='#2b3034', MUT='#9aa1a7', BROWN='#a06a30', GREEN='#3a9a4a', WOOD='#cf9f63';
            ...
            function T(x,y,s,wt,sz,col,align,base){ ctx.font = wt+' '+sz+'px Poppins, Arial, sans-serif'; ctx.fillStyle=col; ctx.textAlign=align||'left'; ctx.textBaseline=base||'top'; ctx.fillText(s,x,y); }
            function Tw(s,wt,sz){ ctx.font = wt+' '+sz+'px Poppins, Arial, sans-serif'; return ctx.measureText(s).width; }
            function drawIco(name,x,y,size){ const im=ico[name]; if(im) ctx.drawImage(im,x,y,size,size); }

            ctx.fillStyle = WHITE; ctx.fillRect(0,0,W,H);
            rrect(16,16,W-16,H-16,22,GRAY_BG,BORD,2);
            const L=16+PAD, R=W-16-PAD, top=16+PAD;

            const RW=Math.round((R-L)*0.55), ry0=top, rh=400;
            rrect(L,ry0,L+RW,ry0+rh,14,WHITE,'#e7e9eb',2);
            if (snap) { const aW=RW-30,aH=rh-30,sc=Math.min(aW/snap.width,aH/snap.height),sw=snap.width*sc,sh=snap.height*sc; ctx.drawImage(snap,L+(RW-sw)/2,ry0+(rh-sh)/2,sw,sh); }

            let dx=L+RW+34, dy=top;
            const mount=(details.shelfTypeValue||details.shelfType||'').toLowerCase();
            const title = (mount.includes('wisz')||mount.includes('hang')) ? 'Półka wisząca'
                        : (mount.includes('stoj')||mount.includes('stand')) ? 'Półka stojąca na blacie'
                        : (mount.includes('mug')||mount.includes('kub')) ? 'Półka na kubki'
                        : (mount.includes('modu')) ? 'Półka modułowa' : (details.shelfType||'Półka');
            T(dx,dy,title,'700',34,TXT); dy+=46;
            T(dx,dy,'PROSTA · FUNKCJONALNA · UNIWERSALNA','500',13,MUT); dy+=34;
            const dimS=(details.width||details.totalWidth||'')+' × '+(details.height||'')+' × '+(details.depth||'')+' cm';
            const pw=Tw(dimS,'500',20)+78;
            rrect(dx,dy,dx+pw,dy+44,11,WHITE,'#dfe2e5',2);
            drawIco('linijka',dx+8,dy+5,34); T(dx+50,dy+22,dimS,'500',20,'#5c636a','left','middle'); dy+=64;
            const colsArr=[['KOLOR BOKÓW',details.sideColor||'—',details.sideColorValue||WOOD],['KOLOR PÓŁEK',details.shelfColor||'—',details.shelfColorValue||WOOD]];
            for(const c of colsArr){
                ctx.beginPath(); ctx.arc(dx+17,dy+17,17,0,7); ctx.fillStyle=c[2]; ctx.fill(); ctx.lineWidth=3; ctx.strokeStyle=WHITE; ctx.stroke();
                ctx.lineWidth=1; ctx.strokeStyle=BORD; ctx.beginPath(); ctx.arc(dx+17,dy+17,18,0,7); ctx.stroke();
                T(dx+46,dy+2,c[0],'500',12,MUT); T(dx+46,dy+16,c[1],'500',17,TXT); dy+=46;
            }
            dy+=6; ctx.strokeStyle=BORD; ctx.lineWidth=2; ctx.beginPath(); ctx.moveTo(dx,dy); ctx.lineTo(R,dy); ctx.stroke(); dy+=14;
            const isStanding = mount.includes('stoj')||mount.includes('stand');
            const feats=[['paczka','Złożona do wysyłki'],[isStanding?'tarcza':'wkretarka', isStanding?'Gotowa do postawienia na blacie':'Montaż na 2 wkręty']];
            for(const f of feats){ drawIco(f[0],dx,dy,40); T(dx+50,dy+20,f[1],'500',17,TXT,'left','middle'); dy+=50; }

            const py0=top+rh+26, ph=180;
            rrect(L,py0,R,py0+ph,14,WHITE,'#e7e9eb',2);
            let lx=L+26, ly=py0+30;
            T(lx,ly,'PŁYTA LAMINOWANA','700',20,TXT); T(lx,ly+28,'18 mm PREMIUM','700',20,BROWN);
            T(lx,ly+70,'Producent:','400',13,MUT); T(lx,ly+88,'KRONOSPAN','700',16,'#1d4e89');
            ctx.strokeStyle='#eceef0'; ctx.lineWidth=2; ctx.beginPath(); ctx.moveTo(lx+330,py0+24); ctx.lineTo(lx+330,py0+ph-24); ctx.stroke();
            let mx=lx+370; T(mx+150,py0+22,'WŁAŚCIWOŚCI','700',13,BROWN,'center');
            const props=[['kropla','Odporna','na wilgoć'],['iskry','Łatwa','w czyszczeniu'],['tarcza','Trwała','i wytrzymała'],['listek','Przyjazna','środowisku']];
            props.forEach((p,i)=>{ const px=mx+i*90; drawIco(p[0],px,py0+44,58); T(px+29,py0+112,p[1],'500',13,'#5c636a','center'); T(px+29,py0+128,p[2],'500',13,'#5c636a','center'); });
            let gx=mx+390;
            T(gx,py0+44,'GRUBOŚĆ','500',13,'#5c636a'); T(gx,py0+62,'PŁYTY','500',13,'#5c636a'); T(gx,py0+86,'1,8 cm','700',20,BROWN);
            const bx=gx+110, by=py0+44, bw=100, bh=70;
            if(plyta){ ctx.save(); rrect(bx,by,bx+bw,by+bh,8,null,null); ctx.clip(); const sc=Math.max(bw/plyta.width,bh/plyta.height); ctx.drawImage(plyta,bx-(plyta.width*sc-bw)/2,by-(plyta.height*sc-bh)/2,plyta.width*sc,plyta.height*sc); ctx.restore(); rrect(bx,by,bx+bw,by+bh,8,null,'#c79a5b',2); }
            else { rrect(bx,by,bx+bw,by+bh,8,'#d9b074','#c79a5b',2); ctx.fillStyle='#b88c4c'; ctx.fillRect(bx,by+bh-16,bw,16); }

            let fy=py0+ph+22; ctx.strokeStyle=BORD; ctx.lineWidth=2; ctx.beginPath(); ctx.moveTo(L,fy); ctx.lineTo(R,fy); ctx.stroke(); fy+=18;
            T(L,fy,'KOD KONFIGURACYJNY','500',12,MUT);
            ctx.font='18px "Courier New",monospace'; ctx.fillStyle='#3a3f44'; ctx.textAlign='left'; ctx.textBaseline='top'; ctx.fillText(details.code||'—',L,fy+18);
            ...
            const rabat=totalRaw*disc, doZap=totalRaw-rabat;
            const fmt=(n)=> n.toFixed(2).replace('.',',')+' zł';
            T(R-300,fy,'RABATY','500',12,MUT); T(R-300,fy+18,'−'+fmt(rabat),'500',18,GREEN);
            ctx.strokeStyle=BORD; ctx.lineWidth=2; ctx.beginPath(); ctx.moveTo(R-180,fy-2); ctx.lineTo(R-180,fy+50); ctx.stroke();
            T(R,fy,'ŁĄCZNIE DO ZAPŁATY','500',12,MUT,'right'); T(R,fy+20,fmt(doZap),'700',30,GREEN,'right');
...
async function generateCompositeFromTemplate(shelves, tpl) {
    ...
    // Pasek płyty laminowanej (img/pasek.jpg) doklejany pod kartą
    const _pasekImg = await new Promise(res => { const i = new Image(); i.onload = () => res(i); i.onerror = () => res(null); i.src = 'img/pasek.jpg'; });
    ...
    const _paskW = W - 60;
    const _paskH = _pasekImg ? Math.round(_paskW * _pasekImg.height / _pasekImg.width) : 0;
    const _paskGap = _paskH ? 18 : 0;
    const _contentH = Math.round(pageH * SC);
    const H = _contentH + (_paskH ? _paskH + _paskGap : 0);
    ...
    ctx.textAlign = 'left';
    // Doklej pasek płyty laminowanej pod kartą (na szerokość karty)
    if (_pasekImg) { ctx.drawImage(_pasekImg, 30, _contentH + _paskGap, _paskW, _paskH); }
...
async function startCompositeExport() {
    ...
    try {
        ...
        for (let i = 0; i < codes.length; i++) {
            ...
            await reconfigureFromDetails(details);
            // Poczekaj aż animacja składania FAKTYCZNIE się skończy (nie sztywne 900 ms),
            // żeby zrzut nie złapał półki w trakcie składania.
            {
                const _t0 = Date.now();
        ...
        successRow.innerHTML = `
            ...
            <button id="compositeDownloadBtn" style="padding:9px 16px;background:#16a34a;color:#fff;border:none;border-radius:9px;font-size:12px;font-weight:700;cursor:pointer;">↓ JPG zbiorczy</button>
            <button id="compositePdfBtn" style="padding:9px 16px;background:#2563eb;color:#fff;border:none;border-radius:9px;font-size:12px;font-weight:700;cursor:pointer;">↓ PDF do druku</button>
        `;
        document.getElementById('batchItemsContainer').appendChild(successRow);
        document.getElementById('compositeDownloadBtn').onclick = () => {
            const url = URL.createObjectURL(blob);
        ...
        document.getElementById('compositePdfBtn').onclick = () => openPrintReport(shelves);
...
${sumHTML}
<svg width="${SVG_W}" height="${SVG_H}" viewBox=